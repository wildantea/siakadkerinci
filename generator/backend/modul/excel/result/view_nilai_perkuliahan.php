...
<button class="btn btn-primary btn-flat download-data-nilai_perkuliahan"><i class="fa fa-cloud-download"></i> Download Data</button>
...
<!--start filter block -->
<div id="download-filter-nilai_perkuliahan" style="display:none">
  <form class="form-horizontal">
    
        <div class="form-group">
          <label for="Jurusan" class="control-label col-lg-1" style="text-align:right;">sem_id</label>
          <div class="col-lg-3">
            <select id="sem_id_nilai_perkuliahan" name="sem_id_nilai_perkuliahan" data-placeholder="Pilih Jurusan ..." class="form-control chzn-select" tabindex="2">
        ...
        <div class="form-group">
          ...
          <div class="col-lg-3">
            <select id="kode_dikti_nilai_perkuliahan" name="kode_dikti_nilai_perkuliahan" data-placeholder="Pilih Jurusan ..." class="form-control chzn-select" tabindex="2">
              <option value="all">Semua</option> <?php
                  foreach ($db->query("select * from tabe_name") as $isi) {
                      echo "<option value="$isi->kode_jur">$isi->nama_jur</option>";
                   } ?>
    ...
    <div class="form-group">
      ...
      <div class="col-lg-3">
        <span class="btn btn-success btn-flat download-data-nilai_perkuliahan-now">
...
$(".download-data-nilai_perkuliahan").on("click", function() {
  if ($("#download-filter-nilai_perkuliahan").is(":visible")){
    $("#download-filter-nilai_perkuliahan").hide();
  } else {
    $("#download-filter-nilai_perkuliahan").show();
  }
});

$(".download-data-nilai_perkuliahan-now").on("click", function() {
    total_down_nilai_perkuliahan();
});


function total_down_nilai_perkuliahan() {
    $("#show_progress").show();
    var totaldata;
    var start_time = new Date().getTime();
        
    var datas = {
        sem_id : $("#sem_id_nilai_perkuliahan").val(),
				kode_dikti : $("#kode_dikti_nilai_perkuliahan").val()
    }

     $.ajax({
      url: "<?=base_admin();?>modul/nilai_perkuliahan/stream_nilai_perkuliahan.php?ask=jumlah",
      type : "post",
      dataType: "json",
      "async":false,
      data : datas,
      success: function(data) {
          ...
          var bagi = Math.ceil(total_data/200);
          getDataDownnilai_perkuliahan(bagi,total_data,start_time);
      }
    });
    ...
    var counters = 0;
    var persen = 0;
    var progress_down=200;
    var last = "";
    
window.getDataDownnilai_perkuliahan=function(bagi,total_data,start_time)
{

    console.log(total_data);
    var start = start_time;
    if ((bagi*200)==progress_down) {
        data = {
            offset : counters,
    ...
    $.ajax({
        ...
        url: "<?=base_admin();?>modul/nilai_perkuliahan/stream_nilai_perkuliahan.php",
        //async:false,
        data : data,
        type : "post",
        dataType: "json",
            ...
            proses(persen);
              ...
              //console.log(data[index].offset);
               if (counters < total_data) {
                  getDataDownnilai_perkuliahan(bagi,total_data,start);
                } else {
                 $("#loadnya").hide();
                  var end_time = new Date().getTime();
                  waktu = "Total Waktu Upload : "+millisToMinutesAndSeconds(end_time-start);
                  alert("Download Data nilai_perkuliahan Selesai");
