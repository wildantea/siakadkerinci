...
<button class="btn btn-primary btn-flat download-data-mahasiswa"><i class="fa fa-cloud-download"></i> Download Data</button>
...
<!--start filter block -->
<div id="download-filter-mahasiswa" style="display:none">
  <form class="form-horizontal">
    
        <div class="form-group">
          <label for="Jurusan" class="control-label col-lg-1" style="text-align:right;">kode_dikti</label>
          <div class="col-lg-3">
            <select id="kode_dikti_mahasiswa" name="kode_dikti_mahasiswa" data-placeholder="Pilih Jurusan ..." class="form-control chzn-select" tabindex="2">
        ...
        <div class="form-group">
          ...
          <div class="col-lg-3">
            <select id="mulai_smt_mahasiswa" name="mulai_smt_mahasiswa" data-placeholder="Pilih Jurusan ..." class="form-control chzn-select" tabindex="2">
              <option value="all">Semua</option> <?php
                  foreach ($db->query("select * from tabe_name") as $isi) {
                      echo "<option value="$isi->kode_jur">$isi->nama_jur</option>";
                   } ?>
    ...
    <div class="form-group">
      ...
      <div class="col-lg-3">
        <span class="btn btn-success btn-flat download-data-mahasiswa-now">
...
$(".download-data-mahasiswa").on("click", function() {
  if ($("#download-filter-mahasiswa").is(":visible")){
    $("#download-filter-mahasiswa").hide();
  } else {
    $("#download-filter-mahasiswa").show();
  }
});

$(".download-data-mahasiswa-now").on("click", function() {
    total_down_mahasiswa();
});


function total_down_mahasiswa() {
    $("#show_progress").show();
    var totaldata;
    var start_time = new Date().getTime();
        
    var datas = {
        kode_dikti : $("#kode_dikti_mahasiswa").val(),
				mulai_smt : $("#mulai_smt_mahasiswa").val()
    }

     $.ajax({
      url: "<?=base_admin();?>modul/mahasiswa/stream_mahasiswa.php?ask=jumlah",
      type : "post",
      dataType: "json",
      "async":false,
      data : datas,
      success: function(data) {
          ...
          getDataDownmahasiswa(bagi,total_data,start_time);
      }
    });
...
window.getDataDownmahasiswa=function(bagi,total_data,start_time)
{
    ...
    $.ajax({
        ...
        url: "<?=base_admin();?>modul/mahasiswa/stream_mahasiswa.php",
        //async:false,
        data : data,
        type : "post",
        dataType: "json",
            ...
            proses(persen);
              ...
              //console.log(data[index].offset);
               if (counters < total_data) {
                  getDataDownmahasiswa(bagi,total_data,start);
                } else {
                 $("#loadnya").hide();
                  var end_time = new Date().getTime();
                  waktu = "Total Waktu Upload : "+millisToMinutesAndSeconds(end_time-start);
                  alert("Download Data mahasiswa Selesai");
