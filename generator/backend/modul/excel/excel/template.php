...
$data_rec = array();
$filename = \'../../../upload/sample/'.$service_name_underscore.'/error_'.$service_name_underscore.'.xlsx\';
...
$filename = \''.$service_name_underscore.'.xlsx\';
header(\'Content-disposition: attachment; filename="\'.XLSXWriter::sanitize_filename($filename).\'"\');
header("Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
header(\'Content-Transfer-Encoding: binary\');
header(\'Cache-Control: must-revalidate\');
header(\'Pragma: public\');
$writer->writeSheet($data_rec,\'Data '.$service_name.'\', $header, $style);
...
<div class="box box-primary">
            ...
            <div class="box-body">
           <form class="form-horizontal" id="filter_form" method="post" action="<?=base_admin();?>modul/'.$service_name_underscore.'/download_data.php" target="_blank">
                   '.$group_filter.'
...
$(document).ready(function(){
        ...
        $("#import_data").click(function() {
          $.ajax({
              url : "<?=base_admin();?>modul/'.$service_name_underscore.'/import_data.php",
              type : "GET",
...
$(\'#filter\').on(\'click\', function() {
  dtb_'.$service_name_underscore.' = $("#dtb_'.$service_name_underscore.'").DataTable({

            "order": [[1,\'asc\']],  
            destroy : true,
    
            \'ajax\':{
              url :\'<?=base_admin();?>modul/'.$service_name_underscore.'/'.$service_name_underscore.'_data.php\',
    ...
    $("#filter_form").validate({
        ...
        rules: {
         ...
         messages: {
          ...
          sem_filter: {
          required: "Untuk Cetak Data Silakan Pilih excel_file",
...
?>
        <div class="alert isi_informasi" style="display:none;margin-bottom: 0px;">
          <button type="button" class="close" data-dismiss="alert">&times;</button>
                     ...
                     <form id="input_import" method="post" class="form-horizontal foto_banyak" action="<?=base_admin();?>modul/'.$service_name_underscore.'/'.$service_name_underscore.'_action.php?act=import">
                     <div class="form-group">
                        <label for="nim" class="control-label col-lg-2">&nbsp;</label>
                        <div class="col-lg-10"> <a href="<?=base_url();?>upload/sample/'.$service_name_underscore.'/template_'.$service_name_underscore.'.xlsx" class="btn btn-success btn-flat"><i class="fa fa-cloud-download"></i> Download Contoh Template XLS</a></div>
                      </div><!-- /.form-group -->
                      <div class="form-group">
                   ...
                   $(form).ajaxSubmit({
                       ...
                       //  enctype:  \'multipart/form-data\'
                        success: function(data){
                          ...
                          dtb_'.$service_name_underscore.'.draw( false );
                          $(\'#loadnya\').hide();
  ...
  foreach ($Reader as $key => $val)
  {
    ...
    if ($key>0) {
      ...
      if ($val[0]!=\'\') {
          ...
          $check = $db->checkExist(\''.$main_table.'\',$check_data);
          if ($check==true) {
            $error_count++;
            ...
            /**
             *if (is_int(trimmer($val[46]))) {
                  $dosen_pemb = trimmer($val[46]);
                } else {
                  $dosen_pemb = 0;
                }
             */
...
if (!empty($data_insert)) {
    $insert = $db->insertMulti(\''.$main_table.'\',$data_insert);
...
if (($sukses>0) || ($error_count>0)) {
    ...
    <font color=\"#3c763d\">".$sukses." data '.$service_name.' berhasil di import</font><br />
    ...
    if ($error_count>0) {
        ...
        $msg .= "</div><br><a href=\'".base_url()."upload/sample/'.$service_name_underscore.'/error_'.$service_name_underscore.'.xlsx\' class=\'btn btn-sm btn-primary\' style=\'text-decoration:none;\'>Download Data Error</a><br>";
    }
