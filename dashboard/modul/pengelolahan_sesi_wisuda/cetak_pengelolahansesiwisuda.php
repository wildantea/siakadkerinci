...
<?php
  ...
  if(isset($_POST['id_wisuda'])) {
    if($_POST['id_wisuda'] != 'all') {
      $priode = ' and kw.id_wisuda="'.$_POST['id_wisuda'].'"';
    }
  }
  ...
  $qq=$db->query("select *,ta.status_ta, kw.id_wisuda,ta.nim,m.nama,f.nama_resmi,j.nama_jur,ta.id_ta 
...
ta.status_ta=3 $jurusan $priode $fakultas group by id_detail");
foreach ($qq as $kk) {
  # code...
}
?>
<body onLoad="//window.print();">
  <div class="page-break">
    ...
    <br>
      <h2 align="center" style="margin-bottom: 10px;">DAFTAR PESERTA WISUDA</h2>
...
<?php
  ...
  $q=$db->query("select *,ta.status_ta, kw.id_wisuda,ta.nim,m.nama,f.nama_resmi,j.nama_jur,ta.id_ta 
from detail_wisuda kw inner join mahasiswa m on m.nim=kw.nim 
inner join tugas_akhir ta on ta.nim=kw.nim 
inner join fakultas f on ta.kode_fak=f.kode_fak 
inner join jurusan j on ta.kode_jurusan=j.kode_jur WHERE 
ta.status_ta=3 $jurusan $priode $fakultas group by id_detail");
  ...
  $status="";
  foreach ($q as $k) {
    if($k->status_wisuda == 1){
      $status="<font color='green'>Sudah Bayar</font>";
    } else{
