<!-- Content Header (Page header) -->
                <section class="content-header">
                    <h1>
                        ...
                        <li><a href="<?=base_index();?>pendaftaran-beasiswa">Beasiswa</a></li>
                        <li class="active">Beasiswa List</li>
                    </ol>
                </section>
                            ...
                            <div class="box">
                                ...
                                <?php
                                  foreach ($db->fetch_all("sys_menu") as $isi) {
                                      ...
                                      <a id="add_jenis_beasiswa" class="btn btn-primary "><i class="fa fa-plus"></i> Tambah Beasiswa</a>        
                                      <?php
                                          }
                                      }
                                  }
        ...
        <?php
            ...
            if ($role_act['del_act']=='Y') {
                $del = "<button data-id='+aData[indek]+' data-uri=".base_admin()."modul/pendaftaran_beasiswa/pendaftaran_beasiswa_action.php?act=delete_beasiswa".' class="btn btn-danger hapus_data"><i class="fa fa-trash" data-toggle="tooltip" title="Hapus"></i></button>';
            } else {
                $del="";
            }
                             }
    ...
    <div class="modal" id="modal_edit_pendaftaran_beasiswa" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"> <div class="modal-dialog modal-lg"> <div class="modal-content"><div class="modal-header"> <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button> <h4 class="modal-title">Edit Beasiswa</h4> </div> <div class="modal-body" id="isi_edit_pendaftaran_beasiswa"> </div> </div><!-- /.modal-content --> </div><!-- /.modal-dialog --> </div>
      ...
      $("#add_jenis_beasiswa").click(function() {
          $.ajax({
              url : "<?=base_admin();?>modul/pendaftaran_beasiswa/beasiswa_add.php",
              type : "GET",
    ...
    $(".table").on('click','.edit_data',function(event) {
        ...
        $.ajax({
            url : "<?=base_admin();?>modul/pendaftaran_beasiswa/beasiswa_edit.php",
            type : "post",
            data : {id_data:id},
            success: function(data) {
    ...
    $(".table").on('click','.hapus_data',function(event) {
        ...
        $.ajax({
            url : "<?=base_admin();?>modul/pendaftaran_beasiswa/pendaftaran_beasiswa_action.php?act=delete_beasiswa",
            type : "post",
            data : {id_data:id},
            success: function(data) {
              $("#loadnya").hide();
      ...
      var dataTable = $("#dtb_pendaftaran_beasiswa").DataTable({
           ...
           'columnDefs': [ {
            ...
            'ajax':{
              url :'<?=base_admin();?>modul/pendaftaran_beasiswa/beasiswa_data.php',
