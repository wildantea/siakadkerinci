...
<button class="btn btn-danger" name="laporan" type="submit" style="float:right"><i class="fa fa-print" ></i> Cetak Semua Jadwal</button>
 <input name='sem2' type='hidden' value='<?= de($_GET['sem']) ?>'>
    ...
    &nbsp;&nbsp;<button class="btn btn-primary" data-toggle="modal" data-target="#myModal" type="submit" style="float:right"><i class="fa fa-print"></i> Cetak Jadwal</button>&nbsp;&nbsp;
             ...
             <a  style="float:right;cursor:pointer" onclick="reset_jadwal()" class="btn btn-success "><i class="fa  fa-eraser"></i> Reset Jadwal</a>&nbsp;&nbsp;&nbsp;&nbsp;
             
              <?php
          $cek_generate = $db->query("select * from semester where kode_jur='".$dec->dec($_GET['jur'])."'
                                     and id_semester='".de($_GET['sem'])."' ");
          foreach ($cek_generate as $c) {
            if ($c->generate_jadwal=='N') {
              ?>
               <a href="<?= base_index()."jadwal-kuliah/generate_jadwal/".$_GET['jur']."/".$_GET['sem'] ?>" style="float:right;margin-right:3px" class="btn btn-success "><i class="fa fa-gears"></i> Generate Jadwals</a>&nbsp;&nbsp;<br><br><br>
              <?php
            }
          }
          ?>
           <a href="<?= base_index()."jadwal-kuliah/generate_jadwal/".$_GET['jur']."/".$_GET['sem'] ?>" style="float:right;margin-right:3px" class="btn btn-success "><i class="fa fa-gears"></i> Generate Jadwald</a>&nbsp;&nbsp;<br><br><br>
