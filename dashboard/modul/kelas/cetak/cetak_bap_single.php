...
$jumlah_data = $db2->fetchCustomSingle("select count(*) as jml from tb_data_kelas_pertemuan where kelas_id=?",array('kelas_id' => $_POST['kelas_id']));
...
?>
  ...
  <tr>
    ...
    <!-- <th>PRODI</th> -->
    <th nowrap="nowrap">PARAF</th>
    <th nowrap="nowrap">KET MHS</th>

  </tr>
            <?php
            //pertemuan 
            $meets = $db2->query("select tb_data_kelas_pertemuan.*,isi_absensi,(select materi from rps_materi_kuliah where id_kelas=tb_data_kelas_pertemuan.kelas_id and pertemuan=tb_data_kelas_pertemuan.pertemuan) as materi from tb_data_kelas_pertemuan left join tb_data_kelas_absensi using(id_pertemuan) where kelas_id=? limit $offset,$limit",array('kelas_id' => $_POST['kelas_id']));
            $cek_jml_pertemuan = $db2->fetchCustomSingle("select count(*) from tb_data_kelas_pertemuan where kehadiran_dosen!='' and kelas_id=?",array('kelas_id' => $_POST['kelas_id']));
            if ($meets->rowCount()>0) {
...
?>
    ...
    <?php
    if ( $meet->tanggal_pertemuan.' '.$meet->selesai < date('Y-m-d H:i:s') ) {
      if ($meet->kehadiran_dosen!="") {
         // Link untuk QR
        $link_qr = "https://siakad.iainkerinci.ac.id/status_absen.php?id=".$meet->id_pertemuan;
    ...
    ?>
    <img src="data:image/png;base64,<?= $imageString ?>" width="60" height="60" alt="QR">
    <?php
     }
?>
...
if ($meet->kehadiran_dosen!="") {
  $data = json_decode($meet->isi_absensi, true);
...
?>
 <td style="vertical-align: middle;  line-height: 1.6; padding-left: 10px;">
  <?php
   if ( $meet->tanggal_pertemuan.' '.$meet->selesai < date('Y-m-d H:i:s') ) {
    ?>
      Hadir: <?=$counts['Hadir'];?><br>
        Sakit: <?=$counts['Sakit'];?><br>
        Ijin: <?=$counts['Ijin'];?><br>
        Alpha : <?=$counts['Alpa'];?>
    <?php
  }
         ...
         ?>
          ...
          <table class="table table-bordered table-striped display nowrap" width="100%" id="dtb_pengesahan_dokumen">
              ...
              <tr>  
                ...
                <?php
                   if ($cek_jml_pertemuan->jml==16) {
                    // Link untuk QR
                      $link_qr = "https://siakad.iainkerinci.ac.id/status_absen_all.php?id=".$meet->kelas_id;
