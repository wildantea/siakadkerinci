...
$header_attributes = $db->fetchCustomSingle("SELECT vk.kode_jur,vk.kls_nama,vk.total_sks as sks,vk.kode_mk,vk.nama_mk,vjk.nama_hari,vjk.jam_mulai,vjk.jam_selesai,nm_ruang,fungsi_dosen_kelas(vk.kelas_id) as nama_dosen,vk.nama_jurusan,view_semester.tahun_akademik,jml_pertemuan,vk.kelas_id from view_kelas vk
...
<body class="A4 landscape">
   <?php
    ...
    $count_peserta = $db->fetchCustomSingle("SELECT COUNT(tdk.krs_detail_id) as jml  from tb_data_kelas_krs_detail tdk WHERE disetujui=1 AND kelas_id=?",array('kelas_id' => $_POST['kelas_id']));
              ...
              $row_mahasiswa=$db->query("SELECT view_simple_mhs.nim,view_simple_mhs.nama from tb_data_kelas_krs_detail tdk
inner join tb_data_kelas_krs using(krs_id)
INNER JOIN view_simple_mhs USING(nim)
WHERE tdk.disetujui=1 AND kelas_id=? order by nim asc limit $offset,$limit",array('kelas_id' => $header_attributes->kelas_id));
      ...
      <?php
       ...
       <div style="text-align: right;margin-top: -20px;"><?=$kode_dokumen;?></div>
           ...
           <td rowspan = "6" width ="35%" valign="top">
             <table>
              ...
              <td>
                ...
                if (hasFakultas()) {
                  ?>
                  <h4>FAKULTAS <?= getFakultasName($header_attributes->kode_jur) ?></h4>
                  <?php
                }
                ?>
        ...
        <tr>
               ...
               <td nowrap ="nowrap" >:</td>
               <td colspan="2"><?= $header_attributes->kode_mk."/".$header_attributes->nama_mk ?></td>
            </tr>
            <tr>
               ...
               <td nowrap ="nowrap" >:</td>
               <td nowrap ="nowrap" ><?= $header_attributes->tahun_akademik ?></td>
            </tr>
            <tr>
            ...
            <tr>
               ...
               <td nowrap ="nowrap"  valign="top"><?= $header_attributes->nama_hari ?> / <?= $header_attributes->jam_mulai  ?>-<?= $header_attributes->jam_selesai ?></td>
 ...
 <br />
         <table width="100%" class="tabel-common">
           ...
           // if ($row_mahasiswa->rowCount()>0) {
            ...
            <tr>
               <th rowspan="3" >No.</th>
               <th rowspan="3" width="10%">NIM</th>
               <th rowspan="3" width="20%">Nama</th>
               <th colspan="<?=$header_attributes->jml_pertemuan;?>">Perkuliahan Ke / Tanggal</th>
               <th rowspan="2" colspan="3">Rekapitulasi</th>
            </tr>
            <tr>
              <?php
              for ($head_pertemuan=1; $head_pertemuan <= $header_attributes->jml_pertemuan; $head_pertemuan++) { 
            ...
            <tr>
              <?php
              for ($body_pertemuan=1; $body_pertemuan <= $header_attributes->jml_pertemuan; $body_pertemuan++) { 
               echo '<th width="4%">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</th>';
              }
              ?>
              <th>Target</th>
         ...
         ?>
              ...
              <?php
               ...
               if ($header_attributes->nama_dosen!="") {
                 ...
                 ?>
                 <tr>
                    <th colspan="<?=$header_attributes->jml_pertemuan+6;?>">DOSEN PENGAMPU</th>
                  </tr>
                     <?php
                 if (count($dosen_pengampu)>0) {
            ...
            <tr valign="center">
                  <td colspan="3" height="34" style="text-align:center; vertical-align:middle;" width="100">Jumlah Mahasiswa Hadir</td>
                     <?php
                              for ($row_mhs_pertemuan=1; $row_mhs_pertemuan <= $jumlah_pertemuan; $row_mhs_pertemuan++) { 
                               echo '<td width="4%">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>';
                              }
                     ?>
                     <td></td><td></td><td></td>
