...
$header_attributes = $db->query("SELECT vk.kode_jur,vk.kls_nama,vk.total_sks as sks,vk.kode_mk,vk.nama_mk,vjk.nama_hari,vjk.jam_mulai,vjk.jam_selesai,nm_ruang,vk.nama_jurusan,view_semester.tahun_akademik,vk.kelas_id,
...
//tanggal uts
$tanggal_uts = $db->fetchCustomSingle("select tgl_uts from tb_data_semester_prodi where id_semester=? and kode_jur=?",array('id_semester' => $_POST['periode'],'kode_jur' => $_POST['jur_filter']));
...
WHERE tdk.disetujui=1 AND kelas_id=? order by nama asc limit $offset,$limit",array('kelas_id' => $header_attribute->kelas_id));
      ...
      <h2 align="center">DAFTAR PESERTA UJIAN<br>DAN NILAI UJIAN  TENGAH SEMESTER</h2>
                  ...
                  <table class="tabel-info">
                    <tbody><tr>
                      <td>Nama Kelas</td>
                      <td>:</td>
                      <td style="width:50%"><?= $header_attribute->kls_nama ?></td>
                      <td>Ruang</td>
                      <td>:</td>
                      <td><?= $header_attribute->nm_ruang ?></td>
                    </tr>
                    <tr>
                      <td>Matakuliah</td>
                      <td>:</td>
