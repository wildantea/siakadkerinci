...
(select count(id_pertemuan) as jml_pertemuan from tb_data_kelas_pertemuan where kelas_id=krs_detail.id_kelas) as jml_pertemuan
 ...
 where id_kelas='$kelas_id' and disetujui='1'",$columns);
  ...
  $ResultData[] = $datatable2->number($i);
    ...
    if ($value->jml_pertemuan>0) {
      $absen = ' (<b> '.round(((get_absen($kelas_id,$value->nim)/$value->jml_pertemuan)*100),2).'%</b>)';
    } else {
      $absen = '';
    }
