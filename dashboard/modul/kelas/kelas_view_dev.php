...
<!-- Content Header (Page header) -->
                <section class="content-header">
                    <h1>
                        ...
                        <li><a href="<?=base_index();?>kelas"> Kelas & Jadwal Mengajar</a></li>
                        <li class="active"> Kelas & Jadwal Mengajar</li>
                    </ol>
                </section>
...
<div class="alert alert-info alert-dismissible">
                ...
                Halaman ini berisi daftar dari kelas dan jadwal yang Bapa & Ibu ajar. Klik pada nama Kelas untuk melihat detail dari Kelas. 
              </div>
...
//check waktu cetak
$check_pengaturan = $db2->fetchCustomSingle("select * from tb_data_korelasi_survey where jenis='bap_presensi'");
if ($check_pengaturan->id_jenis_survei!="") {
    $id_survey = $check_pengaturan->id_jenis_survei;
$hasil_survey = $db2->query("select id,nama_survey from tb_survey_kat where id in($id_survey) and id not in(select id_jenis_survey from tb_survey_hasil where nim='$user' and id_semester='". getSemesterAktif()."' and jenis='bap_presensi')");
    if ($hasil_survey->rowCount()>0) {
        $is_survey = 0;
        foreach ($hasil_survey as $survey) {
            $button_survey .= '<a class="btn btn-primary isi-survey" data-toggle="tooltip" data-title="Untuk bisa Cetak BAP atau Presensi, Silakan Isi Kuesioner Ini" data-id="'.$survey->id.'"><i class="fa fa-p fa-hand-pointer-o"></i> Isi '.$survey->nama_survey.'</a> ';
        }

    }
}
$cetak_bap_presensi = "<a data-id='+data+' data-toggle=\"tooltip\" title=\"Cetak\" class=\"cetak\"><i class=\"fa fa-print\"></i> Cetak Data</a>";
if ($is_survey==0) {
    echo ' <div class="alert alert-danger" role="alert" id="warning">
                  <strong><i class="fa fa-warning"></i></strong> Untuk bisa Cetak BAP atau Presensi, Silakan isi Kuesioner dibawah</h1>
                </div>';
    echo $button_survey;
    $cetak_bap_presensi = "";
}
...
<div class="box box-primary">
   <div class="box-header with-border">
           ...
           <form class="form-horizontal" id="filter_kelas_form" method="post" action="<?=base_admin();?>modul/kelas/cetak.php" target="_blank">
                      <div class="form-group">
                        ...
                        <select id="sem_filter" name="periode" data-placeholder="Pilih Semester ..." class="form-control chzn-select" tabindex="2" required="">
                          <?php 
                          $periodes = $db2->query("select sem_id from view_jadwal_dosen_kelas
where id_dosen=? group by sem_id order by sem_id desc",array('nip' => $_SESSION['username']));
                          foreach ($periodes as $periode) {
                            if ($periode->sem_id==getSemesterAktif()) {
                               echo "<option value='".$periode->sem_id."' selected>".getPeriode($periode->sem_id)."</option>";
                            } else {
                               echo "<option value='".$periode->sem_id."'>".getPeriode($periode->sem_id)."</option>";
                            }
                      ...
                      <input type="hidden" name="id_dosen" value="<?=$_SESSION['username'];?>">
                      <!-- /.form-group -->
                      <div class="form-group">
 ...
 <div class="alert alert-warning fade in error_data_delete" style="display:none">
        ...
        <div class="table-responsive">
                        <table id="dtb_kelas_jadwal" class="table table-bordered table-striped display nowrap" width="100%">
                            <thead>
                                <tr>
                                <th>No</th>
                                  <th>Matakuliah</th>
                                  ...
                                  <th>Jam</th>
                                  <th>Pengajar</th>
                                  <th><a data-toggle="tooltip" data-title="Jumlah Peserta Kelas">PST</a></th>
                                  <th>Program Studi</th>
                                  <th>Action</th>
                                </tr>
                            </thead>
...
$(".isi-survey").click(function(event) {
        $("#loadnya").show();
        ...
        $.ajax({
            url : "<?=base_admin();?>modul/kelas/isi_survey.php",
            type : "post",
            data : {id_survey:id},
      ...
      if ($(".filter-body").is(':visible')) {
        ...
        var dtb_kelas_jadwal = $("#dtb_kelas_jadwal").DataTable({
         ...
         //disable order dan searching pada tombol aksi use "className":"none" for always responsive hide column
            ...
            'ajax':{
              url :'<?=base_admin();?>modul/kelas/kelas_data_dev.php',
...
$(".table").on('click','.peserta-kelas',function(event) {
        ...
        $.ajax({
            url : "<?=base_admin();?>modul/kelas/modal_view.php",
            type : "post",
