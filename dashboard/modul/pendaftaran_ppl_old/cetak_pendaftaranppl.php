...
<?php
  ...
  $qq=$db->query("select p.nim,m.nama,f.nama_resmi,j.nama_jur,p.id from ppl p 
    inner join mahasiswa m on p.nim=m.nim 
    inner join fakultas f on p.kode_fak=f.kode_fak 
    inner join jurusan j on p.kode_jurusan=j.kode_jur
    where p.id is not null $fakultas $jurusan group by p.id");
foreach ($qq as $kk) {
  # code...
}
?>
<body onLoad="//window.print();">
  <div class="page-break">
    ...
    <br>
      <h2 align="center" style="margin-bottom: 10px;">DAFTAR PESERTA PPL</h2>
      ...
      <table class="tabel-common" width="100%">
        <tbody>
          <tr>
            ...
            <th width="100" scope="col">Nama Perusahaan</th>
          </tr>         
<?php
    
  $q=$db->query("select p.nim,m.nama,f.nama_resmi,j.nama_jur,d.nama_dosen,p.judul_kp,p.nama_perusahaan,p.id from ppl p 
    inner join mahasiswa m on p.nim=m.nim 
    inner join fakultas f on p.kode_fak=f.kode_fak 
    inner join jurusan j on p.kode_jurusan=j.kode_jur
    inner join dosen d on p.pembimbing_kam=d.id_dosen
    where p.id is not null $fakultas $jurusan group by p.id");
