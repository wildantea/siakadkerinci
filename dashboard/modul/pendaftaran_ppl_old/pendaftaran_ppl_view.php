<!-- Content Header (Page header) -->
                            ...
                            <div class="box">
                                ...
                                <?php
                                  foreach ($db->fetch_all("sys_menu") as $isi) {
                                      ...
                                      ?>
                                      <a href="<?=base_index();?>pendaftaran-ppl/tambah" class="btn btn-primary "><i class="fa fa-plus"></i> <?php echo $lang["add_button"];?></a>
                                      <?php
                                          }
                                      }
                                  }
 ...
 <div class="alert alert-warning fade in error_data_delete" style="display:none">
        ...
        <?php
            ...
            if (uri_segment(1)==$isi->url) {
              ...
              if ($role_act["up_act"]=="Y") {
                $edit = "<a data-id='+aData[indek]+' href=".base_index()."pendaftaran-ppl/edit/'+aData[indek]+' class=\"btn btn-primary btn-sm edit_data \" data-toggle=\"tooltip\" title=\"Edit\"><i class=\"fa fa-pencil\"></i></a>";
              } else {
                  $edit ="";
              }
            if ($role_act['del_act']=='Y') {
                $del = "<button data-id='+aData[indek]+' data-uri=".base_admin()."modul/pendaftaran_ppl/pendaftaran_ppl_action.php".' class="btn btn-danger hapus_dtb_notif btn-sm" data-toggle="tooltip" title="Hapus" data-variable="dtb_pendaftaran_ppl"><i class="fa fa-trash"></i></button>';
            } else {
                $del="";
            }
                             }
      ...
      var dtb_pendaftaran_ppl = $("#dtb_pendaftaran_ppl").DataTable({
           "fnCreatedRow": function( nRow, aData, iDataIndex ) {
            var indek = aData.length-1;
            $('td:eq('+indek+')', nRow).html('<a href="<?=base_index();?>pendaftaran-ppl/detail/'+aData[indek]+'"  class="btn btn-success btn-sm" data-toggle="tooltip" title="Detail"><i class="fa fa-eye"></i></a> <?=$edit;?> <?=$del;?>');
...
/* Add a click handler for the delete row */
  $('#bulk_delete').click( function() {
    var anSelected = fnGetSelected( dtb_pendaftaran_ppl );
