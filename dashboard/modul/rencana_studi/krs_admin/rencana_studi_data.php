...
$columns = array(
    ...
    'mahasiswa.nama',
    'mulai_smt',
    'view_nama_gelar_dosen.nama_gelar',
    'total_tagihan',
    'disetujui',
    'jatah_sks',
    'sks_diambil',
...
IFNULL(
    ...
    mahasiswa.jur_kode,view_nama_gelar_dosen.nama_gelar,
 ...
 $fakultas $jur_kode $mulai_smt $is_bayar $disetujui");
  ...
  $query = $datatable2->execQuery("select mahasiswa.nim,nama,mulai_smt,
    ...
    (SELECT MIN(disetujui) 
     ...
     WHERE nim = mahasiswa.nim AND id_semester = '".$_POST['periode']."'), 
    0
  ) AS disetujui,mahasiswa.jur_kode,view_nama_gelar_dosen.nama_gelar,
...
0 as is_krs,
 ...
 from mahasiswa 
 left join view_nama_gelar_dosen on mahasiswa.dosen_pemb=view_nama_gelar_dosen.nip
where mahasiswa.nim not in(select nim from krs_detail where id_semester='".$_POST['periode']."') and mahasiswa.status='M'
and nim not in(select nim from tb_data_kelulusan where tb_data_kelulusan.nim=mahasiswa.nim and '".$_POST['periode']."' > semester )
$check_jumlah
$fakultas $jur_kode $mulai_smt $is_bayar $disetujui
",$columns);
...
/*    if ($value->is_bayar=="" || $value->is_bayar>0) {
...
/*    if ($value->sisa_tagihan > 0 && $value->affirmasi=="" && $value->sisa_tagihan < $value->nominal_akhir_tagihan) {
    ...
    if ($value->is_krs=='1') {
      $ResultData[] = '<a href="'.base_index_new().'rencana-studi/detail/?n='.$enc->enc($value->nim).'&s='.$enc->enc($periode).'" data-toggle="tooltip" data-id="'.$value->id_krs_detail.'" title="" class="btn btn-primary btn-xs data_selected_id" data-original-title="Lihat Detail Rencana Studi"><i class="fa fa-eye"></i> Detail</a>';
    } else {
      if ($krs == 1) {
        $ResultData[] = '<a href="'.base_index_new().'rencana-studi/detail/?n='.$enc->enc($value->nim).'&s='.$enc->enc($periode).'" data-toggle="tooltip" title="" class="btn btn-primary btn-xs data_selected_id" data-original-title="Tambah Rencana Studi"><i class="fa fa-plus"></i></a>';
      } else {
        if (in_array($_SESSION['group_level'],$array_allowed_login_as)) {
          $ResultData[] = '<a href="'.base_index_new().'rencana-studi/detail/?n='.$enc->enc($value->nim).'&s='.$enc->enc($periode).'" data-toggle="tooltip" title="" class="btn btn-primary btn-xs data_selected_id" data-original-title="Tambah Rencana Studi"><i class="fa fa-plus"></i></a>';
        } else {
