...
$disetujui_default = 1;
$disetujui = $db->fetch_custom_single("select fungsi_get_jatah_sks(".$_SESSION['username'].",".get_sem_aktif().") as jatah_sks,fungsi_jml_sks_diambil(".$_SESSION['username'].",".get_sem_aktif().") as diambil,fungsi_get_krs_disetujui_mhs(".$_SESSION['username'].",".get_sem_aktif().") as status_disetujui");
// echo "select fungsi_get_jatah_sks(".$_SESSION['username'].",".get_sem_aktif().") as jatah_sks,fungsi_jml_sks_diambil(".$_SESSION['username'].",".get_sem_aktif().") as diambil,fungsi_get_krs_disetujui_mhs(".$_SESSION['username'].",".get_sem_aktif().") as status_disetujui";

$disetujui_default = $disetujui->status_disetujui;

$sks_diambil = sksDiambilMhs($_SESSION['username'],get_sem_aktif());

    //check semester mahasiswa
    $semester_mhs = $db->fetch_custom_single("select count(akm_id) as semester from akm where mhs_nim=?",array('mhs_nim' => $_SESSION['username']));
    $check_paket_semester = $db->fetch_single_row("data_paket_semester","id",1);
    $data_jatah_sks = $db->fetch_custom_single("select fungsi_get_jatah_sks('".$_SESSION['username']."','".get_sem_aktif()."') as jatah_sks");
  ...
  foreach ($query as $value) {
    ...
    if ($disetujui_default<1 && $is_periode) {
