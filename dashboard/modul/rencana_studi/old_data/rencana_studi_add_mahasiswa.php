...
<!--   <button id="cetak_krs" data-sem="<?=$enc->enc(get_sem_aktif());?>" data-nim="<?=$enc->enc($data_mhs->nim);?>" class="btn btn-primary" style="float:right"><i class="fa fa-print"></i>&nbsp;Cetak KRS</button> -->
  ...
  <input type="hidden" id="jatah_jml" value="<?= $jatah_sks->jatah_sks; ?>">
  <input type="hidden" id="diambil_jml" value="<?= $jatah_sks->diambil; ?>">
  <input type="hidden" id="sisa_jml" value="<?= $jatah_sks->jatah_sks - $jatah_sks->diambil; ?>">
                                       ...
                                       if ($disabled_select=='') {
                                          ...
                                          ?>
                                              ...
                                              <?php
                                                  if ($cek_prasyarat=="0") {
                                                    ?>
                                                    <select class="kelas_name" style="width:100px" <?= $disabled_select ?> data-name="kelas_<?= $k->id_matkul ?>" name="kelas[]" id="kelas-<?= $k->id_matkul ?>" disabled>
                                                              ...
                                                              if ($show_kelas_full==false) {
                                                                if (cek_kelas_penuh($kk->kelas_id)==false || cek_sudah_ambil2($data_mhs->nim,get_sem_aktif(),$k->id_matkul,$kk->kelas_id)['result']==$kk->kelas_id ) {
                                                                  if (cek_sudah_ambil2($data_mhs->nim,get_sem_aktif(),$k->id_matkul,$kk->kelas_id)['result']==1 ) {
                                                                    echo "<option value='$kk->kelas_id' >$kk->nm_paralel $kk->kls_nama</option>";
                                                                  }else{
                                                                     echo "<option value='$kk->kelas_id'>$kk->nm_paralel $kk->kls_nama</option>";
                                                                  }
                                                                }
                                                                 ...
                                                                 if (cek_sudah_ambil2($data_mhs->nim,get_sem_aktif(),$k->id_matkul,$kk->kelas_id)['result']==1 ) {
                                                                    echo "<option value='$kk->kelas_id' >$kk->kls_nama</option>";
                                                                  }else{
                                                                     echo "<option value='$kk->kelas_id'>$kk->kls_nama</option>";
                                                                  }
                                                              }
                                     ...
                                     <td colspan="2"><a href="<?=base_index();?>rencana-studi" class="btn btn-default" id="btn-simpan"><i class="fa fa-backward"></i> Kembali</a>  
                                      <button type="submit" class="btn btn-primary btn_simpan_krs"><i class="fa fa-save"></i> Simpan </button></td>
                                        ...
                                        <div id="info" class="alert alert-warning alert-info" style="position:fixed;bottom:5px;right:5px;font-size:20px" role="alert">
                                          Jatah SKS : <?= $jatah_sks->jatah_sks; ?> <br>
                                          Sisa Jatah SKS   : <span id="diambil_info" style="font-size:20px">  <?= $jatah_sks->jatah_sks - $jatah_sks->diambil ?> </span>
                                        </div>
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content" >
        ...
        <div class="modal-footer">
          <button type="button" class="btn btn-success" onClick="window.location='<?=base_index();?>rencana-studi/tambah'">Close</button>
