            ...
            if ($val[0]!='') {
              ...
              if ($check_nim==false) {
                ...
                $check_kode_mk = $db->fetch_custom_single("select bobot_minimal_lulus,sum(matkul.sks_tm+matkul.sks_prak+matkul.sks_prak_lap+matkul.sks_sim) as sks, matkul.kode_mk,matkul.id_matkul from matkul inner join kurikulum on matkul.kur_id=kurikulum.kur_id
                    where trim(kode_mk)=? and trim(kurikulum.kode_jur)=?",array(
                      'kode_mk' => $kode_mk,
                      'kode_jur' => $kode_jur
                    )
                  );
                  if ($check_kode_mk==false) {
                    ...
                    $error_count++;
                      ...
                      //check kelas exist
                      $check_kelas = $db->fetch_custom_single("select * from kelas where trim(sem_id)=? and id_matkul=? and trim(kls_nama)=?",array(
                          'sem_id' => $semester,
                      ...
                      if ($check_kelas==false) {
                        ...
                        if ($check_krs!=false) {
                          ...
                          $array_import[] = array(
                            ...
                            'disetujui' => '1',
                            'batal' => 0,
                            'sks' => $check_kode_mk->bobot_minimal_lulus,
...
if (!empty($array_import)) {
 ...
 //  print_r($_POST);
   ...
   $sudah_diambil = $db->fetch_custom_single("select group_concat(id_kelas) as id_kelas_ambil from krs_detail where nim=? and id_semester=?",
    array('nim' => $_POST['nim'],'id_semester' => $_POST['semester']));

   //get attribut for krs
   if (!empty($_POST['kelas'])) {
      foreach ($_POST['kelas'] as $id_kelas) {
            ...
            $data_kelas_check = $db->fetch_single_row("view_jadwal","kelas_id",$id_kelas);
                  ...
                  if ($sudah_diambil) {
                    //check bentrok
                    $loop_jadwal_exist = $db->query("select * from view_jadwal where kelas_id in(".$sudah_diambil->id_kelas_ambil.")");
                    foreach ($loop_jadwal_exist as $krs_exist) {
                ...
                if ($has_error==0) {
                    if (cek_kelas_penuh($id_kelas)==false) {
                       //$in =  $db->insert('krs_detail',$data);
                      $data_krs = array(
                        'kode_mk' => $data_kelas_check->id_matkul,
                        'id_kelas' => $data_kelas_check->kelas_id,
                        'sks' => get_jumlah_sks_matkul($data_kelas_check->id_matkul),
                        'nim' => $_POST['nim'],
                      ...
                      $in = $db->insert('krs_detail',$data_krs);
                      $sukses .= "- Mata kuliah ".$data_kelas_check->matkul_jadwal." kelas ".$data_kelas_check->kelas_jadwal." Berhasil diambil <i class='fa fa-check'></i><br>";

                    }
                    else{
                       $error .= "- Mata kuliah ".$data_kelas_check->matkul_jadwal." kelas ".$data_kelas_check->kelas_jadwal." sudah penuh <i class='fa fa-remove'></i><br>";
                    }
                }
...
//echo "$id";
         // die();
          ...
          if ($_POST['aksi']=='1') {
            ...
            $qs=$db->query("select ma.jur_kode, ma.mulai_smt, k.sks,m.id_tipe_matkul,m.sks_tm from krs_detail k
            left join matkul m on m.id_matkul=k.kode_mk
            left join mahasiswa ma on ma.nim=k.nim  where k.nim='$nim_sem->nim'
