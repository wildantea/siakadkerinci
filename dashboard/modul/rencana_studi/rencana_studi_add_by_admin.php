...
<section class="content">
      ...
      <div class="box box-solid box-primary">
        ...
        <div class="box-body">
          <div class="box-header">
            ...
            $sem2 = substr($sem, 0, 4);
            // echo "$sem2";
            $semester_mhs = $db->fetch_custom_single("select ((left($sem,4)-left($data_mhs->mulai_smt,4))*2)+right($sem,1)-(floor(right($data_mhs->mulai_smt,1)/2)) as semester from mahasiswa where nim=?", array('mhs_nim' => $data_mhs->nim));
          ...
          <div class="box-body table-responsive">
            ...
            <form method="POST" id="input_rencana_studi"
              action="<?= base_admin() ?>modul/rencana_studi/rencana_studi_action.php?act=add_krs_mhs">
              ...
              <a href="<?= base_index(); ?>rencana-studi/detail/?n=<?= $enc->enc($nim); ?>&s=<?= $enc->enc($sem); ?>"
                class="btn btn-default" id="btn-simpan"><i class="fa fa-backward"></i> Kembali</a>
                  ...
                  foreach ($dtb as $isi) {
                    ...
                    <tr">
                      ...
                      foreach ($dtb2 as $k) {
                        ...
                        if (
                          cek_sudah_ambil($data_mhs->nim, $sem, $k->id_matkul)['result'] == 1
  ...
  <td align="center" >  <label class="mt-checkbox mt-checkbox-single mt-checkbox-outline "> <input type="checkbox" class="group-checkable bulk-check"  name="krs-<?= $k->id_matkul ?>" id="krs-<?= $k->id_matkul ?>" onchange="ambil_krs(this,<?= $sem . "," . $k->id_matkul . "," . $k->sks_tm . "," . $data_mhs->nim ?>)" value="<?= $k->id_matkul ?>" checked> <span></span></label></td>  -->
                          ...
                          cek_sudah_history($data_mhs->nim, $k->id_matkul)['result'] == 1
                          ...
                          <tr style="background:#ff8f00">
                            ...
                            <td align="center">
                              <label class="mt-checkbox mt-checkbox-single mt-checkbox-outline">
                                <input type="checkbox" class="group-checkable bulk-check" name="krs-<?= $k->id_matkul ?>"
                                  id="krs-<?= $k->id_matkul ?>"
                                  onchange="ambil_krs(this, <?= $sem ?>, <?= $k->id_matkul ?>, <?= ($k->sks_tm + $k->sks_prak + $k->sks_prak_lap + $k->sks_sim) ?>, '<?= $data_mhs->nim ?>')"
                                  value="<?= $k->id_matkul ?>">
                                <span></span>
                              </label>
                            </td>
  ...
  <td align="center">  <label class="mt-checkbox mt-checkbox-single mt-checkbox-outline "> <input type="checkbox" class="group-checkable bulk-check"  name="krs-<?= $k->id_matkul ?>" id="krs-<?= $k->id_matkul ?>" onchange="ambil_krs(this,<?= $sem . "," . $k->id_matkul . "," . $k->sks_tm . "," . $data_mhs->nim ?>)" value="<?= $k->id_matkul ?>" checked> <span></span></label></td>  -->
                        ...
                        if ($disabled_select == '') {
                          ...
                          ?>
                            ...
                            <td>
                              ...
                              if ($cek_prasyarat == "0") {
                                ?>
                                <select class="kelas_name" style="width:100px" <?= $disabled_select ?>
                                  data-name="kelas_<?= $k->id_matkul ?>" name="kelas-<?= $k->id_matkul ?>"
                                  id="kelas-<?= $k->id_matkul ?>" disabled>
                            ...
                            <td><span class="all_kuota"></span><input type="hidden"
                                class="kelas_<?= $k->id_matkul ?> reset-doang" name="kelas_<?= $k->id_matkul ?>"
                                value="kelas_<?= $k->id_matkul ?>"></td>
                  ...
                  ?>
                    <tr>
                      <td colspan="2"> <a
                          href="<?= base_index(); ?>rencana-studi/detail/?n=<?= $enc->enc($nim); ?>&s=<?= $enc->enc($sem); ?>"
                          class="btn btn-default" id="btn-simpan"><i class="fa fa-backward"></i> Kembali</a> <button
                          type="submit" class="btn btn-primary btn_simpan_krs"><i class="fa fa-save"></i> Simpan
                        </button></td>
                    </tr>
          ...
          <div id="info" class="alert alert-warning alert-info"
            style="position:fixed;bottom:5px;right:5px;font-size:20px" role="alert">
            Jatah SKS : <?= $jatah_sks; ?> <br>
            Sisa Jatah SKS : <span id="diambil_info" style="font-size:20px"> <?= $jatah_sks - $sks_diambil ?> </span>
          </div>
          <div class="alert alert-danger" style="position:fixed;bottom:5px;right:5px;display:none" role="alert"
            id="warning_krs">
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="myModalLabel" style="text-align:center">Informasi Pengambilan KRS<span
            id="semester"></span></h4>
      </div>
