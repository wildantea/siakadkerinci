    ...
    <div class="row">
      <div class="col-lg-12">
          ...
          <div class="box-header">
            <h3 class="box-title">Pengajuan Rencana Studi Periode <?= get_tahun_akademik(get_sem_aktif()) ?></h3>
  ...
  <div class="box-header">
     ...
     <?php
       //check if current date is periode krs or periode perbaikan krs
                ...
                $check_if_bayar = $db->fetch_custom_single("select fungsi_cek_pembayaran_periode(".get_sem_aktif().",".$data_mhs->jur_kode.",".$data_mhs->nim.") as is_bayar,fungsi_get_krs_disetujui_mhs($data_mhs->nim,".get_sem_aktif().") as status_disetujui,
                  (select ifnull((select nim from affirmasi_krs where periode='".get_sem_aktif()."' and nim='$data_mhs->nim'),'0')) as affirmasi");
                if ($is_periode==false) {
                 ?>
               <div class="alert alert-danger" style="text-align:center" role="alert" id="warning">
                 ...
                 <?php
                } elseif ($check_if_bayar->is_bayar=='0' && $check_if_bayar->affirmasi=='0') {
                  ?>
                <div class="alert alert-danger" style="text-align:center" role="alert" id="warning">
...
?>
                <div class="alert alert-danger" style="text-align:center" role="alert" id="warning">
                  <strong><i class="fa fa-warning"></i></strong> IRS Sudah disetujui Dosen Wali, Silakan Hubungi Dosen Wali untuk merubah IRS
                </div>
                <?php

                } else {
              $jatah_sks = $db->fetch_custom_single("select fungsi_get_jatah_sks(".$data_mhs->nim.",".get_sem_aktif().") as jatah_sks,fungsi_jml_sks_diambil(".$data_mhs->nim.",".get_sem_aktif().") as diambil ");
             ...
             $check_paket_semester = $db->fetch_single_row("data_paket_semester","id",1);
              $data_jatah_sks = $db->fetch_custom_single("select fungsi_get_jatah_sks(".$data_mhs->nim.",".get_sem_aktif().") as jatah_sks,fungsi_jml_sks_diambil(".$data_mhs->nim.",".get_sem_aktif().") as diambil ");
             ...
             $sks_diambil = sksDiambilMhs($data_mhs->nim,get_sem_aktif());

                  ?>


<div class="box-body table-responsive">
<!--   <button id="cetak_krs" data-sem="<?=$enc->enc(get_sem_aktif());?>" data-nim="<?=$enc->enc($data_mhs->nim);?>" class="btn btn-primary" style="float:right"><i class="fa fa-print"></i>&nbsp;Cetak KRS</button> -->
  ...
  <input type="hidden" name="id_semester" value="<?=get_sem_aktif();?>">
    
                                        <a href="<?=base_index();?>rencana-studi" class="btn btn-default" id="btn-simpan"><i class="fa fa-backward"></i> Kembali</a>
                          ...
                          group by sem_matkul order by sem_matkul asc");
                                ...
                                <tr>
                                  ...
                                  <?php
                                    $dtb2=$db->query("select m.*,k.kls_nama from kelas k join matkul m on k.id_matkul=m.id_matkul  join kurikulum ku on ku.kur_id=m.kur_id
                                                     where k.sem_id='".get_sem_aktif()."' and ku.kode_jur='".$data_mhs->jur_kode."' 
                                    ...
                                    foreach ($dtb2 as $k) {
                                     // echo "1<br>";
                                      $cek_prasyarat = cek_prasyarat_mhs($k->id_matkul,$data_mhs->nim);
                                          ...
                                          if (cek_sudah_ambil($data_mhs->nim,get_sem_aktif(),$k->id_matkul)['result']==1
                                            && cek_sudah_ambil($data_mhs->nim,get_sem_aktif(),$k->id_matkul)['nilai']!='' ) {
                                           ?>
                                           ...
                                           <tr style="background:#ff8f00"">
                                            <td align="center" >  
                                              <label class="mt-checkbox mt-checkbox-single mt-checkbox-outline ">
                                               <input type="checkbox" class="group-checkable bulk-check"  name="krs-<?= $k->id_matkul ?>" id="krs-<?= $k->id_matkul ?>" onchange="ambil_krs(this,<?= get_sem_aktif().",".$k->id_matkul.",".($k->sks_tm + $k->sks_prak+$k->sks_prak_lap+$k->sks_sim).",".$data_mhs->nim ?>)" value="<?= $k->id_matkul ?>"> 
                                               <span></span>
                                             </label></td> 
                                           <?php
                                           $ket= "Sudah diambil dan sudah ada nilai untuk matkul ini";
                                          ...
                                          elseif (cek_sudah_ambil($data_mhs->nim,get_sem_aktif(),$k->id_matkul)['result']==1
                                           && cek_sudah_ambil($data_mhs->nim,get_sem_aktif(),$k->id_matkul)['nilai']=='') {
                                             ?>
<!--                                            <tr>
  <td align="center">  <label class="mt-checkbox mt-checkbox-single mt-checkbox-outline "> <input type="checkbox" class="group-checkable bulk-check"  name="krs-<?= $k->id_matkul ?>" id="krs-<?= $k->id_matkul ?>" onchange="ambil_krs(this,<?= get_sem_aktif().",".$k->id_matkul.",".$k->sks_tm.",".$data_mhs->nim ?>)" value="<?= $k->id_matkul ?>" checked> <span></span></label></td>  -->
                                         ...
                                         <!--   <tr style="background:#a91f1f;color:#fff">
                                             ...
                                             $show_kelas_full=true;
                                          } elseif (cek_penuh_permatkul($k->id_matkul,get_sem_aktif())) {
                                            $ket = "";
                                            $disabled_select = "disabled";
                                             $show_kelas_full=true;
                                            // echo "xxxx";
                                            ...
                                            ?>
                                           <tr>
                                    <td align="center"><label class="mt-checkbox mt-checkbox-single mt-checkbox-outline "> <input type="checkbox" class="group-checkable bulk-check" name="krs-<?= $k->id_matkul ?>" id="krs-<?= $k->id_matkul ?>" onchange="ambil_krs(this,<?= get_sem_aktif().",".$k->id_matkul.",".($k->sks_tm + $k->sks_prak+$k->sks_prak_lap+$k->sks_sim).",".$data_mhs->nim ?>)" value="<?= $k->id_matkul ?>"> <span></span></label></td>
                                            <?php
                                            $ket = "Tersedia";
                                            $disabled_select = "";
                                                    ...
                                                    <select class="kelas_name" style="width:100px" <?= $disabled_select ?> data-name="kelas_<?= $k->id_matkul ?>" name="kelas-<?= $k->id_matkul ?>" id="kelas-<?= $k->id_matkul ?>" disabled>
                                                      <option value="">-Pilih Kelas-</option>
                                                        <?php
                                                              ...
                                                              k.id_matkul='$k->id_matkul' and k.sem_id='".get_sem_aktif()."'");
                                                              /*echo "select k.kelas_id,k.kls_nama from kelas k where 
                                                              k.id_matkul='$k->id_matkul' and k.sem_id='".get_sem_aktif()."' <br>";*/
                                                             foreach ($qq as $kk) {
                                                              if ($show_kelas_full==false) {
                                                                if (cek_kelas_penuh($kk->kelas_id)==false || cek_sudah_ambil2($data_mhs->nim,get_sem_aktif(),$k->id_matkul,$kk->kelas_id)['result']==$kk->kelas_id ) {
                                                                  if (cek_sudah_ambil2($data_mhs->nim,get_sem_aktif(),$k->id_matkul,$kk->kelas_id)['result']==1 ) {
                                                                    echo "<option value='$kk->kelas_id===$kk->kls_nama' >$kk->nm_paralel $kk->kls_nama</option>";
                                                                  }else{
                                                                     echo "<option value='$kk->kelas_id===$kk->kls_nama'>$kk->nm_paralel $kk->kls_nama</option>";
                                                                  }
                                                                }
                                ...
                                ?>
                                   <tr>
                                     <td colspan="2"><a href="<?=base_index();?>rencana-studi" class="btn btn-default" id="btn-simpan"><i class="fa fa-backward"></i> Kembali</a>  
                                      <button type="submit" class="btn btn-primary btn_simpan_krs"><i class="fa fa-save"></i> Simpan </button></td>
                                   </tr>
                              
                                        </tbody>                                      
                                         <input type="hidden" name="kode_jur" value="<?= $data_mhs->jur_kode; ?>"> 
                                         <input type="hidden" name="semester" value="<?= get_sem_aktif(); ?>">                                        
                                         </form>
                                      </table>
                                      </div><!-- /.box-body -->
  ...
  <div class="modal modal-success fade" id="myModal" tabindex="-1" role="dialog" data-keyboard="false" data-backdrop="static" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
      <div class="modal-content" >
        ...
        <div class="modal-footer">
          <button type="button" class="btn btn-success" onClick="window.location='<?=base_index();?>rencana-studi'">Close</button>
