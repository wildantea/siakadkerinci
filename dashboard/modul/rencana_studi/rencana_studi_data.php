...
$angkatan_filter = "";
  ...
  if ($_POST['is_bayar']!='all') {
    if ($_POST['is_bayar']=='1') {
      ...
      //Affirmasi
      $is_bayar = 'and (select id_affirmasi from affirmasi_krs where nim=view_krs_single.nim and periode=view_krs_single.id_semester) is not null';
    } elseif ($_POST['is_bayar']=='3') {
      //Belum Lunas
      $is_bayar = 'and (tagihan.sisa_tagihan > 0 and (select id_affirmasi from affirmasi_krs where nim=view_krs_single.nim and periode=view_krs_single.id_semester) is null)';
...
// echo "select view_krs_single.id_semester,id_krs_detail,angkatan,view_krs_single.nim ,tagihan.sisa_tagihan,tagihan.nominal_akhir_tagihan, (select id_affirmasi from affirmasi_krs where nim=view_krs_single.nim and periode=view_krs_single.id_semester limit 1) as affirmasi,  view_krs_single.disetujui,  fungsi_get_jatah_sks(view_krs_single.nim,view_krs_single.id_semester) as jatah,   fungsi_jml_sks_diambil(view_krs_single.nim,view_krs_single.id_semester) as sks_diambil,  nama,jurusan from view_krs_single  inner join mahasiswa on view_krs_single.nim=mahasiswa.nim   inner join view_prodi_jenjang vpj on mahasiswa.jur_kode=vpj.kode_jur   inner join view_semester on  mahasiswa.mulai_smt=view_semester.id_semester left join ( select  ktm.nim,keu_tagihan.nominal_tagihan,nominal_tagihan - ktm.potongan as nominal_akhir_tagihan,nominal_bayar, (keu_tagihan.nominal_tagihan - potongan) - sum(kbm.nominal_bayar) as sisa_tagihan, sum(kbm.nominal_bayar),potongan,syarat_krs,periode FROM keu_tagihan_mahasiswa ktm           JOIN keu_bayar_mahasiswa kbm ON ktm.id=kbm.id_keu_tagihan_mhs           JOIN keu_tagihan ON ktm.id_tagihan_prodi=keu_tagihan.id JOIN keu_jenis_tagihan kjt ON keu_tagihan.kode_tagihan=kjt.kode_tagihan group by ktm.id having   kjt.syarat_krs='Y' ) tagihan on tagihan.nim=view_krs_single.nim and tagihan.periode=view_krs_single.id_semester $jur_filter and view_krs_single.kelas_id is not null $sem_filter   $angkatan_filter $is_bayar $disetuji";
// die();
...
where 1=1 
$jur_filter and view_krs_single.kelas_id is not null $sem_filter   $angkatan_filter $is_bayar $disetuji");


  $query = $datatable2->execQuery("select view_krs_single.id_semester,id_krs_detail,angkatan,view_krs_single.nim ,tagihan.sisa_tagihan,tagihan.nominal_akhir_tagihan, (select id_affirmasi from affirmasi_krs where nim=view_krs_single.nim and periode=view_krs_single.id_semester limit 1) as affirmasi,  view_krs_single.disetujui,  fungsi_get_jatah_sks(view_krs_single.nim,view_krs_single.id_semester) as jatah,   fungsi_jml_sks_diambil(view_krs_single.nim,view_krs_single.id_semester) as sks_diambil,  nama,jurusan from view_krs_single  inner join mahasiswa on view_krs_single.nim=mahasiswa.nim   inner join view_prodi_jenjang vpj on mahasiswa.jur_kode=vpj.kode_jur   inner join view_semester on  mahasiswa.mulai_smt=view_semester.id_semester left join ( select  ktm.nim,keu_tagihan.nominal_tagihan,nominal_tagihan - ktm.potongan as nominal_akhir_tagihan,nominal_bayar, (keu_tagihan.nominal_tagihan - potongan) - sum(kbm.nominal_bayar) as sisa_tagihan, sum(kbm.nominal_bayar),potongan,syarat_krs,periode FROM keu_tagihan_mahasiswa ktm           JOIN keu_bayar_mahasiswa kbm ON ktm.id=kbm.id_keu_tagihan_mhs           JOIN keu_tagihan ON ktm.id_tagihan_prodi=keu_tagihan.id JOIN keu_jenis_tagihan kjt ON keu_tagihan.kode_tagihan=kjt.kode_tagihan group by ktm.id having   kjt.syarat_krs='Y' ) tagihan on tagihan.nim=view_krs_single.nim and tagihan.periode=view_krs_single.id_semester
  where 1=1 $jur_filter and view_krs_single.kelas_id is not null $sem_filter   $angkatan_filter $is_bayar $disetuji",$columns);  
