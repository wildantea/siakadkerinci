...
$nip = $_SESSION['username'];
 ...
 ((left(".$_GET['id_semester'].",4)-left(mulai_smt,4))*2)+right(".$_GET['id_semester'].",1)-(floor(right(mulai_smt,1)/2)) as smt,
 (select foto_user from sys_users where view_simple_mhs_data.nim=sys_users.username) as foto_mhs
  from view_simple_mhs_data
  where nim in(select nim from bimbingan_dosen_pa where nim=? and id_semester=?)
  ",array('nim' => $_GET['nim'],'id_semester' => $_GET['id_semester']));
...
foreach ($data_bimbingan as $bimbingan) {
    $data[] = array(
        ...
        'semester' => $bimbingan->smt.' ('.tampil_periode($_GET['id_semester']).')',
...
<div class="page">
<?php
$link_qr = "https://siakad.iainkerinci.ac.id/bimbingan.php?sem=".$_GET['id_semester'].'&nim='.$m['nim'];
