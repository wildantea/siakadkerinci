      ...
      <div class="box-body filter-body" <?=$display_filter;?>>
       <form class="form-horizontal" id="filter_kelas_form" method="post" action="<?=base_admin();?>modul/pendaftaran/download_data.php" target="_blank">
          ...
          <div class="form-group">
                                ...
                                <?php
                                loopingFakultas('filter_pendaftaran_jadwal');
                                ?>
                      </select>
                    </div>
                              </div><!-- /.form-group -->
          ...
          <div class="form-group">
                                ...
                                <?php
                                loopingProdi('filter_pendaftaran_jadwal',getFilter(array('filter_pendaftaran_jadwal' => 'fakultas')));
                                ?>
                      </select>
                    </div>
              </div><!-- /.form-group -->
                      ...
                      <div class="form-group">
                        ...
                        <select id="periode" name="periode" data-placeholder="Pilih Periode ..." class="form-control chzn-select" tabindex="2" required="">
                          <option value="all">Semua</option>
                          <?php
                            $filter_session_periode = getFilter(array('filter_pendaftaran_jadwal' => 'periode'));
                            foreach ($db2->query("SELECT EXTRACT( YEAR_MONTH FROM `date_created` ) as periode,month(date_created) as bulan,year(date_created) as tahun from tb_data_pendaftaran where tb_data_pendaftaran.status='1' $jur_kode group by EXTRACT( YEAR_MONTH FROM `date_created` ) order by EXTRACT( YEAR_MONTH FROM `date_created` ) desc") as $periode) {
                      ...
                      <div class="form-group">
                        ...
                        <div class="col-lg-10">
                          <span id="filter" class="btn btn-primary"><i class="fa fa-refresh"></i> Filter</span>
                           <?php
                            resetFilterButton('filter_pendaftaran_jadwal');
                            ?>
...
<h4 style="color:#f00">Hanya yang status Sudah Acc pendaftar yang muncul disini
           </h4>
                ...
                <div class="table-responsive">
                        <table id="dtb_jadwal_sidang" class="table table-bordered table-striped display nowrap" width="100%">
                            <thead>
                                <tr>
                                  <th rowspan="2" style='padding-right:7px;' class='dt-center'><label class='mt-checkbox mt-checkbox-single mt-checkbox-outline'> <input type='checkbox' class='group-checkable bulk-check'> <span></span></label></th>
                                  <th rowspan="2" class="none">NIM</th>
                                  ...
                                  <th rowspan="2" class="none">Program Studi</th>
                                  <th rowspan="2" class="none">Judul</th>
