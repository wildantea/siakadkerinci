...
$error_import = $db->query("select * from temp_error_mhs where id_import=?",array('id' => $_GET['id']));
?>
...
foreach ($error_import as $dt) {
	echo "<tr><td>".$no."</td><td>".$dt->nim."</td><td>Nim Sudah ada di Sistem</td></tr>";
$no++;
}
$db->query("delete from temp_error_mhs where id_import=?",array('id' => $_GET['id']));
?>
</table>
