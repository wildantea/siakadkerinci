...
$isi_teks = "NIM : $mhs_id\nNAMA:$data_mhs->nama\nJumlah Bayar : $jml_bayar->jml_bayar\nSemester : $jml_bayar->smt\nUrl : ".base_admin().'modul/riwayat_pembayaran/kuitansi.php?nim='.$_GET['nim'].'&kid='.$_GET['kid'];  
      ...
      <table width="100%">
         <tbody><tr>
            ...
            <td nowrap="nowrap" width="38%"><?= $data_mhs->nama ?></td>
			 <td nowrap="nowrap" width="15%">Nomor Pembayaran</td>
             <td width="2%">:</td>
            <td nowrap="nowrap" width="38%"><?=$jml_bayar->no_kwitansi;?></td>
            <td width="50%" rowspan="6" align="right"><img width="90px" src="temp/<?=$namafile;?>" class="user-image" alt="User Image"/></td>
         </tr>
         <tr>
            <td nowrap="nowrap">NIM</td>
            <td nowrap="nowrap">:</td>
            <td nowrap="nowrap"><?= $mhs_id ?></td>
			  <td nowrap="nowrap">Tanggal Bayar</td>
              <td nowrap="nowrap">:</td>
            <td nowrap="nowrap"><?= tgl_time($jml_bayar->tgl_bayar)?></td>
         </tr>
         <tr>
        <?php
            if ($jml_bayar->nama_singkat!="") {
                $metode = $jml_bayar->nama_singkat;
            } else {
                $metode = 'Bayar Cash';
            }
        ?>
         <td nowrap="nowrap" width="15%">Program Studi</td>
            ...
            <td nowrap="nowrap">Metode Bayar/ Bank</td>
              <td nowrap="nowrap">:</td>
            <td nowrap="nowrap"><?= $metode?></td>
         </tr>
         <tr>
            <td>&nbsp;</td>
            <td></td>
...
((left(keu_tagihan_mahasiswa.periode,4)-left(berlaku_angkatan,4))*2)+right(keu_tagihan_mahasiswa.periode,1)-(floor(right(berlaku_angkatan,1)/2)) as smt,
keu_bayar_mahasiswa.id from keu_bayar_mahasiswa inner join keu_kwitansi on keu_bayar_mahasiswa.id_kwitansi=keu_kwitansi.id_kwitansi inner join keu_tagihan_mahasiswa on keu_bayar_mahasiswa.id_keu_tagihan_mhs=keu_tagihan_mahasiswa.id inner join keu_tagihan on keu_tagihan_mahasiswa.id_tagihan_prodi=keu_tagihan.id inner join keu_jenis_tagihan on keu_tagihan.kode_tagihan=keu_jenis_tagihan.kode_tagihan left join keu_bank on keu_bayar_mahasiswa.id_bank=keu_bank.kode_bank where nim=? and keu_kwitansi.id_kwitansi=? order by periode asc",$array_where);
         ...
         ?>
          <tr>
            <td colspan="4" class="right" align="center"><strong>TOTAL </strong></td>
