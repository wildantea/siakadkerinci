        ...
        <button class="btn btn-default" type="button">Search User !</button>
      </span>
...
<th>Active</th>
      ...
      $limit =10;
        ...
        if (isset($_GET["q"])) {
          ...
          ($_GET["q"], array("username","email","level_name",));
          $search = "and $search_condition";
        }

      $dtb=$pg->query("select sys_users.aktif,sys_users.username,sys_users.email,
        sys_group_users.level_name,sys_users.id from sys_users 
         inner join sys_group_users on sys_users.group_level=sys_group_users.id where sys_users.id is not null and  group_level not in(3,4) $search");
       $no=$pg->Num($limit);
        $count=$pg->Num($limit);
      if ($dtb->rowCount()<1) {
...
if($isi->aktif=='Y') {
    echo '<button class="btn btn-block btn-success btn-sm">Yes</button>';
  } else {
    echo '<button class="btn btn-block btn-danger btn-sm">No</button>';
    }
?></td>
<td>
...
$pg->url=base_index()."user-management?q=".$_GET['q']."&page=";
      ...
      <a href="<?=base_index();?>user-management/tambah" class="btn btn-primary"><i class="fa fa-plus"></i> Add New</a>
