<section class="content-header">
  ...
  <ol class="breadcrumb">
    ...
    <li class="active">Input Nilai PPL</li>
  </ol>
</section>
<style type="text/css">
  .error {
  ...
  <div class="row">
    <div class="col-xs-12">
      <div class="box">
           ...
           ?>
            <form method="POST" action="<?= base_url() ?>dashboard/modul/nilai/nilai_action.php?act=input_nilai_coba" id="form_input_nilai" >
                    ...
                    <table class="table table-bordered table-striped">
                      ...
                      <tbody>
                        <tr>
                            <td style="width: 200px">Lokasi PPL </td>
                    ...
                    <span class="lead"><button type="submit" class="btn btn-success simpan-nilai" disabled=""><i class="fa fa-check"></i> Simpan</button> <a href="<?=base_index();?>input-nilai-ppl" class="btn btn-warning"><i class="fa fa-reply"></i> Batal</a>  Silakan Checklist Nilai Yang akan di Ubah</span>
                    <p>
                ...
                <table  class="table table-bordered table-striped">
                 ...
                 <?php
                     ...
                     // echo "select k.*,m.nama,m.angkatan,m.jur_kode from kkn k join mahasiswa m on m.nim=k.nimwhere id_lokasi='$data_edit->id_lokasi' and id_priode='$data_edit->id_priode' ";
                      $nilai_data = $db->query("select  k.*,dk.nilai_huruf,dk.bobot,dk.nilai_angka,m.nama,m.mulai_smt,m.jk,m.mulai_smt,m.mulai_smt as angkatan,m.jur_kode,j.nama_jur,dk.id_krs_detail,dk.nilai_angka,dk.nilai_huruf from ppl k join mahasiswa m on m.nim=k.nim
                      join jurusan j on j.kode_jur=m.jur_kode
                      left join krs_detail dk on (dk.nim=k.nim and dk.id_kelas='2' ) where  id_lokasi='$data_edit->id_lokasi' and id_priode='$data_edit->id_priode' order by m.nama asc ");
               // echo "select  k.*,dk.nilai_huruf,dk.bobot,dk.nilai_angka,m.nama,m.mulai_smt,m.jk,m.mulai_smt,m.mulai_smt as  ngkatan,m.jur_kode,j.nama_jur,dk.id_krs_detail,dk.nilai_angka,dk.nilai_huruf from ppl k join mahasiswa m on m.nim=k.nim join jurusan j on j.kode_jur=m.jur_kode left join krs_detail dk on (dk.nim=k.nim and dk.id_kelas='2' ) where  id_lokasi='$data_edit->id_lokasi' and id_priode='$data_edit->id_priode' order by m.nama asc";
        ...
        maxlength="5" size="5" data-angkatan="<?=$nilai->mulai_smt;?>" data-jurusan="<?=$nilai->jur_kode;?>"></td>
                          <td>
