...
include "../../inc/config.php";
    $id_lokasi = de($_GET['id']);
     $data_edit = $db->fetch_single_row("v_dpl_ppl","id_lokasi",$id_lokasi); 
         ?>
<style type="text/css">
  table {
...
<section class="content">
  <div class="row" style="display: block;width: 100%">
    ...
    <div>
     ...
     <tr style="margin-top: 10px">
       <td style="width: 150px">Periode PPL</td>
       <td>: <?= ganjil_genap($data_edit->priode) ?></td>
     </tr>
     <tr style="margin-top: 10px">
       <td style="width: 150px">Lokasi PPL</td>
  ...
  <div style="display: block;margin-top: 40px">
        ...
        <table  class="table table-bordered table-striped" border="1" style="width: 85%">
         ...
         <?php
              $no=1;
              foreach ($db->query("select  k.*,dk.nilai_huruf,dk.bobot,dk.nilai_angka,m.nama,m.mulai_smt,m.jk,m.mulai_smt,m.mulai_smt as angkatan,m.jur_kode,j.nama_jur,dk.id_krs_detail,dk.nilai_angka,dk.nilai_huruf from ppl k join mahasiswa m on m.nim=k.nim
                      join jurusan j on j.kode_jur=m.jur_kode
                      left join krs_detail dk on (dk.nim=k.nim and dk.id_kelas='2' ) where  id_lokasi='$data_edit->id_lokasi' and id_priode='$data_edit->id_priode' order by m.nama asc") as $k) {
              echo "<tr>
                      <td>$no</td>
                      <td>$k->nim</td>
