...
<?php
        //get periode input nilai

        $semester_aktif = get_semester_aktif($kelas_attribute->kode_jur); 
      //  echo "select * from semester where kode_jur='$kelas_attribute->kode_jur' and id_semester='$semester_aktif->id_semester'";
      //  $periode = $db->fetch_custom_single("select * from semester where kode_jur=? and id_semester=?",array('kode_jur' => $kelas_attribute->kode_jur,'id_semester' => $semester_aktif->id_semester));
         $periode = $db->fetch_custom_single("select * from semester_ref where id_semester=?",array('id_semester' => $semester_aktif->id_semester)); 
        $current_date = date('Y-m-d');
        if ($current_date<$periode->tgl_mulai_input_nilai || $current_date>$periode->tgl_selesai_input_nilai) {
          echo "<span class='lead'>Maaf Periode Input Nilai Hanya boleh dari tanggal ".tgl_indo($periode->tgl_mulai_input_nilai)." S/d ".tgl_indo($periode->tgl_selesai_input_nilai);
          exit(); 
        }
?>
