...
// where keu_tagihan_mahasiswa.id is not null $kode_prodi $fakultas $mulai_smt $periode $kode_pembayaran $kode_tagihan $status";
  ...
  foreach ($query as $value) {
    ...
    if (in_array($_SESSION['group_level'],$array_allowed_login_as)) {
      $login_as = '<a href="'.base_admin().'inc/login_as.php?id='.$value->nim.'&adm_id='.$_SESSION['id_user'].'&url=mahasiswa&back_uri=setting-tagihan-mahasiswa" class="btn btn-success btn-sm" data-toggle="tooltip" title="" data-original-title="Login As"><i class="fa fa-user"></i></a>';
    }
    if ($value->is_lunas!="") {
