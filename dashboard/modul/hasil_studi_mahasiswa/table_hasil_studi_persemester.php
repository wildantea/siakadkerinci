<div class="box-header">
  <div class="box-group" id="accordion">
    ...
    <?php
      get_nilai_konversi(de(uri_segment(3))); 
 ...
 left join jenis_semester js on js.id_jns_semester=s.id_jns_semester  where k.nim='".de(uri_segment(3))."'  group by k.id_semester          order by s.id_semester asc");
    // echo "select  k.nim,k.id_krs_detail, js.jns_semester,js.nm_singkat, sf.tahun, s.id_semester,a.*, fungsi_jml_sks_diambil(k.nim,k.id_semester) as sks_diambil from krs_detail k left join semester s on s.id_semester=k.id_semester left join akm a on (a.sem_id=s.id_semester and a.mhs_nim=k.nim) left join semester_ref sf on sf.id_semester=s.id_semester join jenis_semester js on js.id_jns_semester=sf.id_jns_semester where k.nim='".de(uri_segment(3))."'  group by k.id_semester          order by s.id_semester asc";
    $i=1;  
    ?>
              ...
              <div class="form-group">
                ...
                <div class="col-lg-10">
                  <select name="sem" id="sem" data-placeholder="Pilih Semester ..." class="form-control chzn-select" tabindex="2" method="post" required>
                   ...
                   <?php
                   //if has no nilai diakui delete akm
                 $akm_sem_pendek = $db2->query("select id_krs_detail from krs_detail where id_semester='10' and nim='$k->nim'");
                  if ($akm_sem_pendek->rowCount()>0) {
                   ...
                   $sem = $db->query("select s.id_semester,js.nm_singkat,js.jns_semester ,sf.tahun from krs_detail k join semester s on s.id_semester=k.id_semester
                    ...
                    where k.nim='".de(uri_segment(3))."' group by k.id_semester order by k.id_semester asc ");
                   foreach ($sem as $isi2) {
  ...
  <div class="panel box box-primary">
    <a style="font-size: 17px" data-toggle="collapse" data-parent="#accordion" href="#<?= $i ?>" class="collapsed" aria-expanded="false">
      <div class="box-header with-border">
        ...
        <div style="float: right;position: relative;top: -25px"> Lihat Detail <i class="fa fa-get-pocket" ></i> </div>
      </div>
    </a>
      ...
      <div class="box-body">
       ...
       <table class="table table-bordered table-striped" >
        ...
        foreach ($qq as $kr) {
         ...
         <td style='text-align:center' id='kredit-$kr->id_krs_detail'>".($kr->bobot*$kr->sks)."</td>
