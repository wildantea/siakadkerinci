...
$jenis_bukti = "";
  ...
  if ($id_jenis_bukti!="") {
    $jenis_bukti = "and id_jenis_bukti in($id_jenis_bukti)";
  }
...
$datatable2->setDebug(1);
  ...
  $query = $datatable2->execQuery("SELECT tb_data_pendaftaran_jenis_bukti.id_jenis_bukti,tb_data_pendaftaran_jenis_bukti.jenis_bukti,type_dokumen,file_name,tb_data_pendaftaran_bukti_dokumen.id_bukti,ext_type,link_dokumen from tb_data_pendaftaran_jenis_bukti
LEFT JOIN tb_data_pendaftaran_bukti_dokumen USING(id_jenis_bukti) inner join tb_data_pendaftaran using(id_pendaftaran) where nim='$nim' $jenis_bukti and id_jenis_pendaftaran_setting='$id_jenis_pendaftaran_setting' and tb_data_pendaftaran.id_pendaftaran='$id_pendaftaran'",$columns);
...
/*        if (strpos($value->link_dokumen, 'drive.google.com')) {
        ...
        $ResultData[] = '<a target="_blank" class="bukti-dokumen title="'.$value->jenis_bukti.'" href="'.$value->link_dokumen.'">'.$value->link_dokumen.'</a>';
      }
    } else {
      $ResultData[] = "";
    }
