...
$mhs_data = $db2->fetchCustomSingle("SELECT tb_data_pendaftaran.id_pendaftaran,tb_data_pendaftaran.ket_ditolak, tb_data_pendaftaran.status,tb_data_pendaftaran.nim,nama,
judul,jurusan as nama_jurusan,angkatan,tb_data_pendaftaran_jenis.nama_jenis_pendaftaran,nama_directory,tb_data_pendaftaran_jenis_pengaturan.* from view_simple_mhs_data
...
<table id="tabelku" class="table table-bordered table-striped" cellspacing="0" width="100%">
   ...
   <div class="alert alert-danger error_data" style="display:none">
          ...
          <span class="isi_warning"></span>
            ...
            <form id="edit_pendaftaran" method="post" class="form-horizontal" action="<?=base_admin();?>modul/pendaftaran/pendaftaran_action.php?act=up">
          ...
          if ($mhs_data->ada_pembimbing=='Y') {
            ...
            $data_pembimbing = $db2->query("select tb_data_pendaftaran_pembimbing.*,nama_gelar from tb_data_pendaftaran_pembimbing 
          inner join view_nama_gelar_dosen on nip=nip_dosen_pembimbing where tb_data_pendaftaran_pembimbing.id_pendaftaran=?",array(
          'id_pendaftaran' => $mhs_data->id_pendaftaran));
            if ($data_pembimbing->rowCount()>0) {
              foreach ($data_pembimbing as $pembimbing) {
                  $pembimbing_mahasiswa[$pembimbing->pembimbing_ke] = array(
                    'nip' => $pembimbing->nip_dosen_pembimbing,
                    'nama_gelar' => $pembimbing->nama_gelar
                  );
              }
            }
          ...
          <label for="Nama Pendaftaran" class="control-label col-lg-2">Pembimbing <?=$i;?> </label>
            <div class="col-lg-10">
              <select id="pembimbing_<?=$i;?>" name="pembimbing[]" data-placeholder="Pilih Dosen Pembimbing <?=$i;?>..." class="form-control pembimbing" tabindex="2" required="">
               <?php
                 echo "<option value='".$pembimbing_mahasiswa[$i]['nip']."'>".$pembimbing_mahasiswa[$i]['nama_gelar']."</option>";
                ?>
              </select>
            </div>
          </div><!-- /.form-group -->
...
if ($mhs_data->has_attr=='Y') {
          ...
          foreach ($attr_value as $attr) {
            ...
            if ($attr->attr_type=="dropdown") {
                ...
                $form_attribute.='
              <div class="form-group">
                    <label for="single" class="control-label col-lg-2">'.$attr->attr_label.'</label>
                    <div class="col-lg-9">
            ...
            $form_attribute.='</select>
              ...
              <div class="form-group">
                  <label for="single" class="control-label col-lg-2">'.$attr->attr_label.'</label>
                   <div class="col-lg-10">
                  <textarea name="'.$attr->attr_name.'" id="'.$attr->attr_name.'" '.$required.' class="form-control">'.$value_edit.'</textarea>
                  </div>
              </div>';
              ...
              <div class="form-group">
                  <label for="single" class="control-label col-lg-2">'.$attr->attr_label.'</label>
                   <div class="col-lg-2">
                  <input type="text" name="'.$attr->attr_name.'" id="'.$attr->attr_name.'" '.$required.' value="'.$value_edit.'" class="form-control tgl_picker">
                  </div>
              </div>';
              ...
              <div class="form-group">
                ...
                <div class="col-lg-3">
                   <div class="input-group date tgl_picker">
                      ...
                      <input type="text" autocomplete="off" class="form-control tgl_picker_input" name="date_created" value="<?=substr($data_edit->date_created,0,10);?>"/>
                   </div>
                </div>
            </div>
