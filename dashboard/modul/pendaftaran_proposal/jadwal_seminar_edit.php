...
$data_edit = $db->fetch_custom_single("select periode_bulan, vp.jurusan,td.kode_jur,td.id as periode_id, tanggal_seminar, id_ruang_seminar,tb_jadwal_seminar.id from tb_jadwal_seminar
...
where tb_jadwal_seminar.id=?",array('id' => $_POST['id_data']))
?>
<style type="text/css"> .datepicker {z-index: 1200 !important; } </style>
 <div class="alert alert-danger error_data" style="display:none">
      ...
      <form id="input_jadwal_ruang" method="post" class="form-horizontal foto_banyak" action="<?=base_admin();?>modul/pendaftaran_proposal/jadwal_seminar_action.php?act=up">
     <div class="form-group">
                                ...
                                <?php
                                echo "<option value='$data_edit->kode_jur'>$data_edit->jurusan</option>";
                                ?>
                      </select>
                    </div>
                                <div class="col-lg-3">
 ...
 <input type="hidden" name="semester_jadwal" id="semester_jadwal" value="<?=$id_semester_aktif;?>">
            ...
            <select name="periode_bulan_jadwal" data-placeholder="Pilih Periode Pedaftaran..." class="form-control chzn-select" tabindex="2" required id="periode_bulan_jadwal">
                  <?php
                        echo "<option value='$data_edit->periode_id'>".bulan_tahun($data_edit->periode_bulan)."</option>";
                         ?>
              </select>
            </div>
...
<div class="form-group">
      ...
      where rr.kode_jur=? and jenis_ruang=?",array("prodi" => $data_edit->kode_jur,'jenis_ruang' => '2'));
             echo "<option value=''>Pilih Ruangan</option>";
            foreach ($data as $dt) {
              print_r($dt);
              if ($data_edit->id_ruang_seminar==$dt->ruang_id) {
                echo "<option value='$dt->ruang_id' selected>$dt->nm_ruang</option>";
              } else {
 ...
 <div class="form-group">
              ...
              <div class="col-lg-3">
                <div class='input-group date' id='tgl'>
                    <input type='text' class="form-control tanggal" name="tanggal_jadwal" value="<?=$data_edit->tanggal_seminar;?>" />
