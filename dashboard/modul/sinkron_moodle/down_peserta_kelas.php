<?php
  ...
  if ($_GET['kode_fak']!='all') {
    ...
    foreach ($q as $k) {
     ...
     header('Content-Type: text/csv; charset=utf-8'); 
      header('Content-Disposition: attachment; filename=peserta_kelas_'.$nama_fak.'_'.$semester.'.csv'); 
  ...
  if ($_GET['kode_jur']!='all' && $_GET['kode_jur']!='') {
    ...
    foreach ($q as $k) {
     ...
     header('Content-Type: text/csv; charset=utf-8'); 
     header('Content-Disposition: attachment; filename=peserta_kelas_'.$nama_jur.'_'.$semester.'.csv'); 
     $output = fopen("php://output", "w");  
  }
  ...
  $q = $db->query("select * from v_peserta_kelas
  ...
  foreach ($q as $k) { 
       $email = $k->email;
       if ($k->email=='') {
         $email = $k->username."@student.iainkerinci.ac.id";
       }
