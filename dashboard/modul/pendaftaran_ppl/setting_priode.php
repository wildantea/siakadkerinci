...
<section class="content-header">
    <h1>
        ...
        <ol class="breadcrumb">
          ...
          <li class="active">Setting Priode PPL</li>
    </ol>
</section>
  ...
  <div class="row">
      ...
      <div class="box">
        ...
        <?php
            ...
            if ($role_act['del_act']=='Y') {
                $del = "<button data-id='+aData[indek]+' data-uri=".base_admin()."modul/pendaftaran_ppl/pendaftaran_ppl_action.php?act=delete_priode".' class="btn btn-danger hapus_data "><i class="fa fa-trash"></i></button>';
            } else {
                $del="";
            }
                             }
    ...
    <div class="modal" id="modal_priode_edit" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"> <div class="modal-dialog modal-lg"> <div class="modal-content"><div class="modal-header"> <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button> <h4 class="modal-title">Edit Priode</h4> </div> <div class="modal-body" id="isi_priode_edit"> </div> </div><!-- /.modal-content --> </div><!-- /.modal-dialog --> </div>
      ...
      $("#add_priode").click(function() {
          ...
          $.ajax({
              url : "<?=base_admin();?>modul/pendaftaran_ppl/priode_add.php",
              type : "GET",
    ...
    function set_aktif(id) {
       $.ajax({
              url : "<?=base_admin();?>modul/pendaftaran_ppl/pendaftaran_ppl_action.php?act=set_aktif_periode",
              type : "POST",
              data : {
                 id : id
              },
    ...
    $(".table").on('click','.edit_data',function(event) {
        ...
        $.ajax({
            url : "<?=base_admin();?>modul/pendaftaran_ppl/priode_edit.php",
            type : "post",
            data : {id_data:id},
            success: function(data) {
    ...
    $(".table").on('click','.hapus_data',function(event) {
        ...
        $.ajax({
            url : "<?=base_admin();?>modul/pendaftaran_ppl/pendaftaran_ppl_action.php?act=delete_priode",
            type : "post",
            data : {id_data:id},
            success: function(data) {
              $("#loadnya").hide();
    ...
    var dataTable = $("#dtb_priode_kkn").DataTable({
               ...
               'columnDefs': [ {
                ...
                'ajax':{
                  url :'<?=base_admin();?>modul/pendaftaran_ppl/priode_data.php',
...
/* Add a click handler for the delete row */
  $('#bulk_delete').click( function() {
    ...
    $('#ucing').modal({ keyboard: false }).one('click', '#delete', function (e) {
        ...
        $.ajax({
            type: 'POST',
            url: '<?=base_admin();?>modul/pendaftaran_ppl/pendaftaran_ppl_action.php?act=del_massal_priode',
