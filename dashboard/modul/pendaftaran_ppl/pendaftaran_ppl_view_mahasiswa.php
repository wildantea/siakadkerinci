...
<section class="content-header">
  ...
  <ol class="breadcrumb">
    ...
    <li><a href="<?=base_index();?>pendaftaran-kukerta">Pendaftaran PPL</a></li>
   ...
   <div class="box">
    <div class="box-body table-responsive">
     ...
     // print_r($_SESSION);
      ...
      $qm = $db->query("select jur_kode from mahasiswa where nim = '".$_SESSION['username']."' ");
      foreach ($qm as $km) {
         $kode_jur= $km->jur_kode;
      }
      
      $qs = $db->query("select syarat_sks,syarat_semester,kondisi from syarat_ppl where kode_jur='$kode_jur' "); 
      $syarat_sudah = true;
      if ($qs->rowCount()>0) {
         foreach ($qs as $ks) {
            $syarat_sks = $ks->syarat_sks;
            $syarat_semester = $ks->syarat_semester;
            $kondisi = $ks->kondisi;
         }
      }else{
          $syarat_semester = "";
      ...
      $sudah_ambil_ppl = cek_sudah_ambil_mk_ppl($nim);
      ...
      $sks_sudah_ambil = get_total_sks($nim);
    //  echo "$status_semester";
     // if($sks <= $total_sks){
      // echo "$status_semester, $sks_sudah_ambil , $sudah_bayar, $sudah_ambil_ppl";
      // var_dump($sudah_ambil_ppl);
      if ($syarat_sudah) {

         if ($kondisi=='1') {
          if ($status_semester>=$syarat_semester && $sks_sudah_ambil>=$syarat_sks && $sudah_bayar=='1' && $sudah_ambil_ppl) { 

      //cek apakah data sudah ada atau belum
        if($check != false){
          $data = $db->query("select pk.priode,lk.nama_lokasi,p.nim,m.nama,f.nama_resmi,j.nama_jur,p.id_kkn from ppl p
 left join mahasiswa m on p.nim=m.nim left join fakultas f on p.kode_fak=f.kode_fak left join jurusan j on p.kode_jur=j.kode_jur left join priode_ppl pk on p.id_priode=pk.id_priode left join lokasi_ppl lk on p.id_lokasi=lk.id_lokasi where m.nim=?",array("cm.nim" => $nim)); 
          foreach ($data as $dt){
            ?>
                    ...
                    <tr>
                      <div class="form-group">
                        <td>
                          ...
                          <a target="_BLANK" href="<?= base_url() ?>dashboard/modul/pendaftaran_ppl/cetak_surat_pernyataan.php?n=<?= en($dt->nim) ?>" class="btn btn-primary"><i class="fa fa-print"> Cetak Surat Pernyataan</i></a>
                        </td>
                      </div>
                    </tr>
                    <tr>
           ...
           // echo "string";
            ...
            foreach ($data as $dt) { 
                ...
                <div class="form-group"> 
                    ...
                    <div class="col-lg-10">
                      <select id="id_lokasi" name="id_lokasi" data-placeholder="Pilih Lokasi Kukerta ..." class="form-control chzn-select" tabindex="2" required="">
                       ...
                       <?php
                       foreach ($db->query("select l.* from lokasi_ppl l left join priode_ppl p on p.id_priode=l.id_periode
               ...
               <div class="form-group" style="display: none">
                ...
                <div class="col-lg-10">
                  <select id="id_priode" name="id_priode" data-placeholder="Pilih Priode Kukerta ..." class="form-control chzn-select" tabindex="2">
                   ...
                   <?php
                   foreach ($db->fetch_all("priode_ppl") as $isi) {
                    if ($isi->aktif=='1') {
             ...
             <div class="form-group">
                ...
                <div class="col-lg-10">
                  <?php
                  $qp = $db->query("select * from priode_ppl where aktif='1' ");
                  foreach ($qp as $kp) {
    ...
    if ($status_semester<$syarat_semester) {
       ?>
    <div class="alert alert-danger" role="alert">
      <article>
        <strong>Perhatian!</strong>  <strong>Status semester anda sekarang adalah semester <?= $status_semester ?>, Kukerta bisa di ambil minimal semester <?= $syarat_semester ?> </strong>
      </article>
    </div> 
    <?php 
    }
    if ($sks_sudah_ambil<$syarat_sks) {
      ?>
    <div class="alert alert-danger" role="alert">
      <article>
        <strong>Perhatian!</strong>  <strong>Minimal SKS yang sudah di ambil untuk mengambil PPL adalah <?= $syarat_sks ?>, sedangkan anda baru mengambil <?= $sks_sudah_ambil ?> SKS </strong>
      </article>
    </div>
    <?php 
    }
     if (!$sudah_ambil_ppl) {
      ...
      <div class="alert alert-danger" role="alert">
        <article>
          <strong>Anda belum mengambil Mata Kuliah PPL </strong>
        </article>
      </div>
      <?php    
    } 
    ...
    <?php 
     ...
     if ($sudah_bayar=='0') { 
      ...
      <?php    
        ...
        <div class="alert alert-danger" role="alert">
      <article>
        <strong>Perhatian!</strong>  <strong>Admin belum melakukan setting syarat untuk syarat PPL anda, silahkan hubungi admin fakultas anda</strong>
      </article>
    </div>
        <?php
      }
