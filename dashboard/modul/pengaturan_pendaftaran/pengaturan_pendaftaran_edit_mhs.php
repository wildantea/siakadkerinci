...
<div class="form-group">
              ...
              <select  id="diperuntukan" class="form-control chzn-select"  name="diperuntukan" tabindex="2" required>
               ...
               <option value="mahasiswa" <?=(uri_segment(3)=='mahasiswa')?'selected':'';?>>Mahasiswa</option>
               <option value="dopeg" <?=(uri_segment(3)=='dopeg')?'selected':'';?>>Dosen/Pegawai</option>
              </select>
          </div>
...
<div class="form-group">
          ...
          <div class="form-group">
              <label for="Ada Jadwal" class="control-label col-lg-2">Ada Matakuliah Harus Di Ambil </label>
              <div class="col-lg-10">
          ...
          <div class="callout callout-info" style="margin-bottom: 5px;">
                          ...
                          <p>Ketika mahasiswa mendaftar pendaftaran ini, maka harus dalam poisi sedang mengambil matakuliah / Krs di Aplikasi Salam. Anda bisa memili matkul multi untuk memastikan matkul yang diambil sesuai kurikulum masing2 mahasiswa</p>
                        </div>
                      </div>
                    </div><!-- /.form-group -->
...
foreach ($db2->query("select kode_jur,nama_jurusan,matkul_syarat from view_prodi_jenjang inner join tb_data_pendaftaran_jenis_pengaturan_prodi using(kode_jur) where id_jenis_pendaftaran_setting=?",array('id_jenis_pendaftaran_setting' => $data_edit->id_jenis_pendaftaran_setting)) as $jurusan) {
...
<?php
$mat = $db2->query("select id_matkul,nama_mk,kode_mk from tb_data_matakuliah where id_matkul in($jurusan->matkul_syarat)");
foreach ($mat as $syarat) {
    echo "<option value='$syarat->id_matkul' selected>$syarat->nama_kurikulum - $syarat->kode_mk $syarat->nama_mk</option>";
}
...
?>
            ...
            <div class="form-group">
                <label for="Ada Judul" class="control-label col-lg-2">Status Mahasiswa Harus Aktif</label>
                <div class="col-lg-10">
                <?php
                if ($data_edit->harus_status_aktif=="Y") {
                ?>
                  <input name="harus_status_aktif" data-on-text="Ya" data-off-text="Tidak" class="make-switch" type="checkbox" checked>
                <?php
              } else {
                ?>
                  <input name="harus_status_aktif" data-on-text="Ya" data-off-text="Tidak" class="make-switch" type="checkbox">
                <?php
              }?>
            ...
            <div class="form-group">
                <label for="Ada Judul" class="control-label col-lg-2">Ada Judul TA/SKRIPSI/TESIS/DISERTASI</label>
                <div class="col-lg-10">
                <?php
                if ($data_edit->ada_judul=="Y") {
                ?>
                  <input name="ada_judul" data-on-text="Ya" data-off-text="Tidak" class="make-switch" type="checkbox" checked id="ada_judul">
                <?php
              } else {
                ?>
                  <input name="ada_judul" data-on-text="Ya" data-off-text="Tidak" class="make-switch" type="checkbox" id="ada_judul">
                <?php
              }?>
            ...
            <div class="form-group show-multi-judul" <?=($data_edit->ada_judul=='N')?'style="display:none"':'';?>>
                <label for="Ada Judul" class="control-label col-lg-2">Multi Judul</label>
                <div class="col-lg-10">
                <?php
                if ($data_edit->ada_judul=="Y" && $data_edit->multi_judul=='Y') {
                ?>
                  <input name="multi_judul" data-on-text="Ya" data-off-text="Tidak" class="make-switch" type="checkbox" checked id="multi_judul">
                <?php
              } else {
                ?>
                  <input name="multi_judul" data-on-text="Ya" data-off-text="Tidak" class="make-switch" type="checkbox" id="multi_judul">
                <?php
              }?>
          ...
          <div class="form-group show-jumlah-judul" <?=($data_edit->multi_judul=='N')?'style="display:none"':'';?>>
              <label for="Jumlah Pembimbing" class="control-label col-lg-2">Jumlah Judul </label>
              <div class="col-lg-1">
                <input type="number" data-rule-number="true" name="jumlah_judul" value="<?=$data_edit->jumlah_judul;?>" class="form-control" id="jumlah_judul" maxlength="1">
              </div>
          </div><!-- /.form-group -->

            <div class="form-group">
                <label for="Ada Pembimbing" class="control-label col-lg-2">Validasi Judul Penasihat Akademik ?</label>
                <div class="col-lg-10">
                <?php if ($data_edit->validasi_judul_pa=="Y") {
                ?>
                  <input name="validasi_judul_pa" data-on-text="Ya" data-off-text="Tidak" class="make-switch" type="checkbox" checked id="validasi_judul_pa">
                <?php
              } else {
                ?>
                  <input name="validasi_judul_pa" data-on-text="Ya" data-off-text="Tidak" class="make-switch" type="checkbox" id="validasi_judul_pa">
                <?php
              }?>
            ...
            <div class="form-group">
                <label for="Ada Pembimbing" class="control-label col-lg-2">Ada Proses Bimbingan Penasihat Akademik</label>
                <div class="col-lg-10">
                <?php if ($data_edit->ada_proses_bimbingan_akademik=="Y") {
                ?>
                  <input name="ada_proses_bimbingan_akademik" data-on-text="Ya" data-off-text="Tidak" class="make-switch" type="checkbox" checked id="ada_proses_bimbingan_akademik">
                <?php
              } else {
                ?>
                  <input name="ada_proses_bimbingan_akademik" data-on-text="Ya" data-off-text="Tidak" class="make-switch" type="checkbox" id="ada_proses_bimbingan_akademik">
                <?php
              }?>
          ...
          <div class="form-group show-pembimbing" <?=$show_pembimbing;?>>
            ...
            <div class="form-group show-isi-pembimbing" <?=$show_pembimbing;?>>
                <label for="Ada Pembimbing" class="control-label col-lg-2">Isi Pembimbing Oleh Mahasiswa ? </label>
                <div class="col-lg-10">
                <?php if ($data_edit->isi_pembimbing_by_mahasiswa=="Y") {
                ?>
                  <input name="isi_pembimbing_by_mahasiswa" data-on-text="Ya" data-off-text="Tidak" class="make-switch" type="checkbox" checked id="isi_pembimbing_by_mahasiswa">
                <?php
              } else {
                ?>
                  <input name="isi_pembimbing_by_mahasiswa" data-on-text="Ya" data-off-text="Tidak" class="make-switch" type="checkbox" id="isi_pembimbing_by_mahasiswa">
                <?php
              }?>
            ...
            <div class="form-group">
                <label for="Ada Pembimbing" class="control-label col-lg-2">Ada Proses Bimbingan </label>
                <div class="col-lg-10">
                <?php if ($data_edit->ada_proses_bimbingan=="Y") {
                ?>
                  <input name="ada_proses_bimbingan" data-on-text="Ya" data-off-text="Tidak" class="make-switch" type="checkbox" checked id="ada_proses_bimbingan">
                <?php
              } else {
                ?>
                  <input name="ada_proses_bimbingan" data-on-text="Ya" data-off-text="Tidak" class="make-switch" type="checkbox" id="ada_proses_bimbingan">
                <?php
              }?>
    ...
    <div class="row row-clone-pemaraf">
        ...
        <span class="input-group-addon numbering drag-handle"><?=$number;?></span>
            <select name="pemaraf[]" data-placeholder="Pilih Validator/Penanda Tangan..." class="form-control pemaraf tgl_picker_input" tabindex="2">
               <?php 
               foreach ($db2->query("select * from tb_data_jabatan_kategori where id_jabatan_kat in(select id_jabatan_kat from tb_data_jabatan_pejabat) and is_editable='Y' union
 select * from tb_data_jabatan_kategori where id_jabatan_kat in(2)") as $isi) {
...
<span class="btn btn-success btn-sm clone-btn"><i class="fa fa-plus"></i> Tambah Pemaraf</span>
 ...
 <div class="form-group" <?=($data_edit->ada_template_surat=='Y') ? '':'style="display:none"';?>>
          ...
          <div class="form-group show-kuesioner" <?=($ada_kuesioner=='Y') ? '' : 'style="display:none"'?>>
                        ...
                        <div class="col-lg-6">
            <select  id="id_jenis_kuesioner" name="id_jenis_kuesioner" data-placeholder="Pilih Jenis Kuesioner ..." class="form-control" tabindex="2">
              <option value=''></option>
              <?php
              foreach ($db2->query("select id,nama_survey from tb_survey_kat where id!='1'") as $isi) {
                if ($data_edit->id_jenis_kuesioner==$isi->id) {
              ...
              ?>
                      ...
                      <hr>
            <div class="form-group">
                <label for="Ada Judul" class="control-label col-lg-2">Ada SK Pembimbing </label>
                <div class="col-lg-5">
                <?php
                if ($data_edit->ada_sk_pembimbing=="Y") {
                ?>
                  <input name="ada_sk_pembimbing" data-on-text="Ya" data-off-text="Tidak" class="make-switch" type="checkbox" checked id="ada_sk_pembimbing">
                  <span class="btn btn-primary btn-sm template-surat-sk-pembimbing edit-template" data-jenis="sk_pembimbing" data-id="<?=$data_edit->id_jenis_pendaftaran_setting;?>"><i class="fa fa-envelope"></i> Edit Template Surat SK Pembimbing</span>
                <?php
              } else {
                ?>
                  <input name="ada_sk_pembimbing" data-on-text="Ya" data-off-text="Tidak" class="make-switch" type="checkbox" id="ada_sk_pembimbing">
                  <span class="btn btn-primary btn-sm template-surat-sk-pembimbing edit-template" style="display:none" data-jenis="sk_pembimbing" data-id="<?=$data_edit->id_jenis_pendaftaran_setting;?>"><i class="fa fa-envelope"></i> Edit Template Surat SK Pembimbing</span>
                <?php
              }?>
                </div>
            </div><!-- /.form-group -->
            ...
            <div class="form-group">
                <label for="Ada Judul" class="control-label col-lg-2">Ada SK Penguji </label>
                <div class="col-lg-5">
                <?php
                if ($data_edit->ada_sk_penguji=="Y") {
                ?>
                  <input name="ada_sk_penguji" data-on-text="Ya" data-off-text="Tidak" class="make-switch" type="checkbox" checked id="ada_sk_penguji">
                  <span class="btn btn-primary btn-sm template-surat-sk-penguji edit-template" data-jenis="sk_penguji" data-id="<?=$data_edit->id_jenis_pendaftaran_setting;?>"><i class="fa fa-envelope"></i> Edit Template Surat SK Penguji</span>
                <?php
              } else {
                ?>
                  <input name="ada_sk_penguji" data-on-text="Ya" data-off-text="Tidak" class="make-switch" type="checkbox" id="ada_sk_penguji">
                  <span class="btn btn-primary btn-sm template-surat-sk-penguji edit-template" style="display:none" data-jenis="sk_penguji" data-id="<?=$data_edit->id_jenis_pendaftaran_setting;?>"><i class="fa fa-envelope"></i> Edit Template Surat SK Penguji</span>
                <?php
              }?>
                </div>
            </div><!-- /.form-group -->
            ...
            <div class="form-group">
                <label for="Ada Penguji" class="control-label col-lg-2">Harus Lunas Pembayaran</label>
                <div class="col-lg-10">
            ...
            <?php
            $show_bayar = "";
            if ($data_edit->harus_lunas_spp=='N') {
              $show_bayar = "style='display:none'";
            }
            ?>
          <div class="form-group show-periode-lunas" <?=$show_bayar;?>>
                        <label for="Bukti" class="control-label col-lg-2">Pilihan Periode/Semester Lunas</label>
                        <div class="col-lg-6">
            <select  id="periode_lunas" name="periode_lunas" data-placeholder="Pilih Periode lunas ..." class="form-control" tabindex="2">
               <option value="all" <?=($data_edit->periode_lunas=='all'?'selected':'');?>>Periode/Semester Awal S/d Akhir Kuliah</option>
                <option value="onlyone" <?=($data_edit->periode_lunas=='onlyone'?'selected':'');?>>Hanya Periode/Semester Aktif Saat Daftar</option>
              </select>
            </div>
                      </div><!-- /.form-group -->

          <div class="form-group show-tagihan" <?=$show_bayar;?>>
                        <label for="Bukti" class="control-label col-lg-2">Pilih Jenis Tagihan</label>
                        <div class="col-lg-10">
            <select id="jenis_tagihan" name="jenis_tagihan_lunas[]" data-placeholder="Pilih Jenis Tagihan Syarat ..." class="form-control chzn-select" tabindex="2" multiple="">
               <option value=""></option>
               <?php 
               $jenis_tagihan_selected = array();
               if ($data_edit->harus_lunas_spp=='Y') {
                 $jenis_tagihan_selected = explode(",", $data_edit->jenis_tagihan_lunas);
               }
               foreach ($db2->fetchAll("keu_jenis_tagihan") as $isi) {
                  if (in_array($isi->kode_tagihan, $jenis_tagihan_selected)) {
 ...
 $.ajax({
   ...
   success : function(data) {
    ...
    $(".edit-template").click(function(){
        ...
        $.ajax({
            url : "<?=base_admin();?>modul/pengaturan_pendaftaran/edit_template_surat.php",
            type : "post",
...
$('#ada_judul').on('switchChange.bootstrapSwitch', function (event, state) {
   if (state==true) {
    $('.show-multi-judul').show();
   } else {
    $('.show-multi-judul').hide();
   }
}); 

$('#multi_judul').on('switchChange.bootstrapSwitch', function (event, state) {
   if (state==true) {
    $('.show-jumlah-judul').show();
    $('.jumlah_judul').prop('required',true);
   } else {
    $('.show-jumlah-judul').hide();
    $('.jumlah_judul').prop('required',true);
   }
}); 
