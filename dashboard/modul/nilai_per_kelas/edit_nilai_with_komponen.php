          ...
          if ($kelas_data->ada_komponen == 'Y') {
            ...
            foreach ($komponen as $key) {
              if (is_array($key)) {
                foreach ($key as $val) {
                  ?>
                  <th style="text-align: center;vertical-align: middle"><?= $val->nama_komponen; ?> (<?= $val->value_komponen; ?>%)
                  </th>
                  <?php
                }
              }
...
use_rule from krs_detail inner join mahasiswa using(nim) where id_kelas=? and krs_detail.disetujui='1' and id_krs_detail in($nilai_id) order by krs_detail.nim asc", array('id_kelas' => $_POST['kelas_id']));
        foreach ($nilai_data as $nilai) {
          ...
          <tr class="row-data">
              ...
              if (is_array($key)) {
                ...
                foreach ($key as $val) {
                  ...
                  <td>
                    ...
                    <input type="text" data-id="<?= $nilai->krs_detail_id; ?>" data-prosentase="<?= $val->value_komponen; ?>"
                      <?= $read_componen_status; ?> name="nilai[<?= $nilai->krs_detail_id; ?>][]" required
            ...
            <td>
              <input type="text" name="nilai_akhir[<?= $nilai->krs_detail_id; ?>]" <?= $read_grade_status; ?>
                value="<?= $nilai_akhir; ?>" data-id="<?= $nilai->krs_detail_id; ?>"
                class="form-control final_grade final_grade_<?= $nilai->krs_detail_id; ?>" maxlength="5" size="5"
                data-jurusan="<?= $kelas_data->kode_jur; ?>" data-angkatan="<?= $nilai->mulai_smt; ?>">
            </td>
            <td>
              <select name="nilai_huruf[<?= $nilai->krs_detail_id; ?>]"
                class="form-control nilai_huruf nilai_huruf_<?= $nilai->krs_detail_id; ?>" <?= $read_huruf_status; ?>>
