        ...
        if (is_array($key)) {
            ...
            foreach ($key as $val) {
                if ($val->id!=1) {
                    ...
                    $last_huruf = $alphas[$i];
                    $rumus.= "(nilai $val->nama_komponen * ".$val->value_komponen." %)+";
                    $i++;
                }
...
//$sheet->setCellValue('F4', "Sistem akan menghitung OTOMATIS NILAI AKHIR berdasarkan rumus ".$rumus); // Set kolom A1 dengan tulisan "DATA SISWA"
//$sheet->mergeCells('F4:I4'); // Set Merge Cell pada kolom A1 sampai F1
$sheet->getStyle('F4')->getFont()->setBold(true); // Set bold kolom A1
$sheet->getStyle('F4')->getFont()->setSize(12); // Set font size 15 untuk kolom A1
...
$spreadsheet->getActiveSheet()->getStyle('A6:'.$last_huruf.'6')->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('92D050');
...
$sheet->getColumnDimension('D')->setWidth(13); // Set width kolom D
