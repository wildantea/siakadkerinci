          ...
          <td><?=$kelas_data->kls_nama ;?> </td>
           ...
           <tr>
            <th style="text-align: right;vertical-align: middle;">Komponen Penilaian</th>
             <th>
<?php
//check if periode input
$is_periode_input = checkPeriodeInput('nilai',$kelas_data->sem_id,$kelas_data->kode_jur);
...
if ($is_periode_input) {
  $bulk_check = '<th rowspan="2" style="padding-right:0;width: 7%;text-align: center;vertical-align: middle;background: #ffffff;color:#000"><label class="mt-checkbox mt-checkbox-single mt-checkbox-outline "> <input type="checkbox" class="group-checkable bulk-check"> <span></span></label>No</th>';
  $notif_input = "Silakan Checklist Nilai Yang akan di Ubah";
} else {
  $bulk_check = '<th rowspan="2">No</th>';
  $notif_input = "";
}
 ?>
      </tbody></table>
...
<div class="alert alert-danger alert-dismissible">
            ...
            <?php
                $check_periode = $db2->fetchCustomSingle("select * from tb_data_semester_prodi s where id_semester='$kelas_data->sem_id' and kode_jur='$kelas_data->kode_jur'");
            ?>
            Saat ini bukan Periode Input Nilai, Input nilai mulai tanggal <?=tgl_indo($check_periode->tgl_mulai_input_nilai);?> s/d <?=tgl_indo($check_periode->tgl_selesai_input_nilai);?>
            </div>
<?php
      }
...
?>
            ...
            <a class="btn btn-success" id="import_data"><i class="fa fa-cloud-upload"></i> Import Excel</a>
...
?>
         ...
         ?>
            ...
            <span class="lead"><span class="btn btn-primary edit-nilai" disabled="" style="display:none"><i class="fa fa-pencil"></i> Edit Nilai</span>  <?=$notif_input;?></span>
            <p>
       </form>
     </div>
...
$("#import_data").click(function() {
          $.ajax({
              url : "<?=base_admin();?>modul/nilai_per_kelas/upload_nilai_komponen.php",
              type : "POST",
              data : {
                kelas_id : <?=$id;?>,
                act : 'kom'
