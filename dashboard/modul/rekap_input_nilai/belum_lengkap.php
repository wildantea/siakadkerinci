...
where j.kode_jur='".$dec->dec(uri_segment(3))."' and kelas.sem_id='".$dec->dec(uri_segment(4))."' and (SELECT COUNT(id_krs_detail) FROM krs_detail k WHERE k.id_kelas=kelas.kelas_id AND (nilai_huruf='') AND k.batal=0 GROUP BY k.id_kelas) IS NOT NULL AND (SELECT COUNT(id_krs_detail) FROM krs_detail k WHERE k.id_kelas=kelas.kelas_id AND (nilai_huruf!='') AND k.batal=0 GROUP BY k.id_kelas) IS NOT NULL");
