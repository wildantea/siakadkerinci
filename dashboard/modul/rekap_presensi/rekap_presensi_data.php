...
foreach ($query as $value) {
    ...
    $cetak = '<a data-id="' . $value->kelas_id . '" data-jadwal="' . $value->jadwal_id . '" data-toggle="tooltip" title="Cetak Presensi/BAP" class="btn btn-sm btn-info cetak"><i class="fa fa-print"></i> Cetak</a>';

    $ResultData[] = "<a class='btn btn-primary btn-sm' data-toggle='tooltip' data-title='Lihat Detail' href='" . base_index_new() . "rekap-presensi/detail/" . $value->kelas_id . "'><i class='fa fa-eye'></i> Detail</a> " . $cetak;
    ;
    ...
    for ($i = 1; $i <= 16; $i++) {
        ...
        if ((int) $value->$col_masuk > 0) {
            ...
            if (
                ...
                $icon_masuk = "<a class='btn btn-default'>
    <i class='fa fa-check' 
       style='color:green' 
       data-toggle='tooltip' 
       data-html='true'
       ...
       title='Masuk: " . $hari_masuk . "<br>" . tgl_indo($value->$col_masuk) . " " . $jam_masuk . "'>
                ...
                //      $icon_masuk = "<a class='btn btn-default'><i class='fa fa-check' style='color:green' data-html='true' data-toggle='tooltip' data-title='Masuk: " . $hari_masuk . ", " . tgl_indo($value->$col_masuk) . " " . $jam_masuk . "'></i></a>";
            } else {
                $icon_masuk = "<a class='btn btn-default'>
    <i class='fa fa-check' 
       style='color:orange' 
