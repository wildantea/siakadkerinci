<!-- Content Header (Page header) -->
                <section class="content-header">
                    <h1>
                        ...
                        <li class="active">Tugas Akhir Jadwal</li>
                    </ol>
                </section>
                                    ...
                                    <div class="row">
                                      <?php
                                        foreach ($db->fetch_all("sys_menu") as $isi) {
                                            ...
                                            ?>
                                            <a href="<?=base_index();?>tugas-akhir" class="btn btn-default"><span class="fa fa-step-backward"></span> Kembali </a>
        ...
        <?php
            ...
            if ($role_act['del_act']=='Y') {
                $del = "<button data-id='+aData[indek]+' data-uri=".base_admin()."modul/tugas_akhir/tugas_akhir_action.php".' class="btn btn-sm btn-danger hapus_dtb hapus_data"><i class="fa fa-trash" data-toggle="tooltip" title="Hapus"></i></button>';
            } else {
                $del="";
            }
                             }
    ...
    <div class="modal" id="modal_tugas_akhir" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"> <div class="modal-dialog modal-lg"> <div class="modal-content"><div class="modal-header"> <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button> <h4 class="modal-title">Penjadwalan Tugas Akhir</h4> </div> <div class="modal-body" id="isi_tugas_akhir"> </div> </div><!-- /.modal-content --> </div><!-- /.modal-dialog --> </div>

    <div class="modal" id="modal_tugas_akhir_edit" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"> <div class="modal-dialog modal-lg"> <div class="modal-content"><div class="modal-header"> <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button> <h4 class="modal-title">Penjadwalan Tugas Akhir Edit</h4> </div> <div class="modal-body" id="isi_tugas_akhir_edit"> </div> </div><!-- /.modal-content --> </div><!-- /.modal-dialog --> </div>
    ...
    $("#add_tugas_akhir").click(function() {
          ...
          $.ajax({
              url : "<?=base_admin();?>modul/tugas_akhir/jadwal_tugas_akhir_add.php",
              type : "GET",
    ...
    $(".table").on('click','.edit_data',function(event) {
        ...
        $.ajax({
            url : "<?=base_admin();?>modul/tugas_akhir/jadwal_tugas_akhir_edit.php",
            type : "post",
            data : {id_data:id},
            success: function(data) {
                $("#isi_tugas_akhir_edit").html(data);
                $("#loadnya").hide();
          }
        });

      $('#modal_tugas_akhir_edit').modal({ keyboard: false,backdrop:'static' });
    ...
    $(".table").on('click','.hapus_data',function(event) {
        ...
        $.ajax({
            url : "<?=base_admin();?>modul/tugas_akhir/tugas_akhir_action.php?act=delete_jadwal",
            type : "post",
            data : {id_data:id},
            success: function(data) {
              $("#loadnya").hide();
      ...
      var dataTable = $("#dtb_tugas_akhir").DataTable({
           ...
           'columnDefs': [ {
            ...
            'ajax':{
              url :'<?=base_admin();?>modul/tugas_akhir/jadwal_tugas_akhir_data.php',
...
/* Add a click handler for the delete row */
  $('#bulk_delete').click( function() {
    ...
    $('#ucing').modal({ keyboard: false }).one('click', '#delete', function (e) {
        ...
        $.ajax({
            type: 'POST',
            url: '<?=base_admin();?>modul/tugas_akhir/tugas_akhir_action.php?act=del_massal_jadwal',
