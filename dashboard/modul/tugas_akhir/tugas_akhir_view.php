<!-- Content Header (Page header) -->
                ...
                <section class="content">
                    <div class="row">
                        <div class="col-xs-12">
                                    ...
                                    <div class="row">
                                      <?php
                                        foreach ($db->fetch_all("sys_menu") as $isi) {
                                            ...
                                            <a id="jadwal_tugas_akhir" class="btn btn-success" href="<?=base_index();?>tugas-akhir/jadwal"><i class="fa fa-calendar"></i> Penjadwalan </a>
                                            <?php
                                                }
                                            }
                                        }
                          ...
                          <a id="cetak_tugas_akhir" class="btn btn-success"><i class="fa fa-print"></i> Cetak Peserta TA</a>
                          <a id="cetak_jadwal" class="btn btn-primary"><i class="fa fa-print"></i> Cetak Jadwal TA</a>
        ...
        <?php
            ...
            if (uri_segment(1)==$isi->url) {
              ...
              if ($role_act["up_act"]=="Y") {
                $edit = "<a data-id='+aData[indek]+'  class=\"btn btn-xs btn-primary edit_data \"><i class=\"fa fa-pencil\" data-toggle=\"tooltip\" title=\"Edit\"></i></a>";
                $setting = "<a data-id='+aData[indek]+' class=\"btn btn-xs btn-warning set_data\"><i class=\"fa fa-gear\" data-toggle=\"tooltip\" title=\"Setting Penguji\"></i></a>";
                $cetak ="<a data-id='+aData[indek]+' class=\"btn btn-xs btn-default cetak_data\"><i class=\"fa fa-print\" data-toggle=\"tooltip\" title=\"Validasi Judul\"></i></a>";
              } else {
                  $validasi="";
                  $edit ="";
                  $setting="";
            ...
            if ($role_act['del_act']=='Y') {
                $del = "<button data-id='+aData[indek]+' data-uri=".base_admin()."modul/tugas_akhir/tugas_akhir_action.php".' class="btn btn-xs btn-danger hapus_dtb "><i class="fa fa-trash" data-toggle="tooltip" title="Hapus"></i></button>';
            } else {
                $del="";
            }
                             }
    ...
    <!-- Modal Cetak Jadwal-->
    <div class="modal" id="modal_jadwal_tugas_akhir" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"> <div class="modal-dialog modal-lg"> <div class="modal-content"><div class="modal-header"> <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button> <h4 class="modal-title">Cetak Jadwal Tugas Akhir</h4> </div> <div class="modal-body" id="isi_jadwal_tugas_akhir"> </div> </div><!-- /.modal-content --> </div><!-- /.modal-dialog --> </div>
    <!-- Modal Tugas Akhir -->
    ...
    <!-- Modal Validasi & Cetak SK-->
    <div class="modal" id="modal_cetak_tugas_akhir" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"> <div class="modal-dialog modal-lg"> <div class="modal-content"><div class="modal-header"> <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button> <h4 class="modal-title">Cetak SK</h4> </div> <div class="modal-body" id="isi_cetak_tugas_akhir"> </div> </div><!-- /.modal-content --> </div><!-- /.modal-dialog --> </div>
    ...
    $("#cetak_jadwal").click(function() {

          $.ajax({
              url : "<?=base_admin();?>modul/tugas_akhir/cetak_jadwal_ta.php",
              type : "GET",
              success: function(data) {
                  $("#isi_tugas_akhir_jad").html(data);
              }
          });

      $('#modal_tugas_akhir_jad').modal({ keyboard: false,backdrop:'static',show:true });
    });
    ...
    $(".table").on('click','.cetak_data',function(event) {
        $("#loadnya").show();
        event.preventDefault();
        var currentBtn = $(this);
        ...
        $.ajax({
            url : "<?=base_admin();?>modul/tugas_akhir/tugas_akhir_val.php",
            type : "post",
            data : {id_data:id},
            success: function(data) {
                $("#isi_cetak_tugas_akhir").html(data);
      ...
      $('#modal_cetak_tugas_akhir').modal({ keyboard: false,backdrop:'static' });
    });
    ...
    $('#modal_tugas_akhir_set').modal({ keyboard: false,backdrop:'static' });
      ...
      var dataTable = $("#dtb_tugas_akhir").DataTable({
           "fnCreatedRow": function( nRow, aData, iDataIndex ) {
            ...
            $('td:eq('+indek+')', nRow).html('<?=$edit;?> <?=$setting;?> <?=$cetak;?> <?=$del;?>');
