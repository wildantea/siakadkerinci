...
where kode_mk=? and kurikulum.kode_jur=?",array(
            ...
            /*    $cek_jadwal_exist = $db->check_exist("jadwal_kuliah",array(
                ...
                if ($cek_jadwal_exist==false) {
                  ...
                  $sukses++;
                  $data_update_jadwal  = array('hari' => $hari ,
                                               'ruang_id' =>  $get_id_ruang->ruang_id,
                                               'jam_mulai' => $jam_mulai,
                                               'jam_selesai' => $jam_selesai );
...
if ($values!="") {
  ...
  $query = "insert into jadwal_kuliah (kelas_id,hari,ruang_id,jam_mulai,jam_selesai) values ".$values;
  //echo $query;
  $db->query($query);
  echo $db->getErrorMessage();
}
...
where kode_mk=? and kurikulum.kode_jur=?",array(
            ...
            if ($check_kelas_exist) {
              ...
              $check_nidn_from_dosen = $db->check_exist("dosen",array('nidn' => $nidn));
              if ($check_nidn_from_dosen==false) {
                $error_count++;
                $error[] = "NIDN/NIDK/NUP ".$nidn." ".$nama_dosen." tidak ditemukan di Sistem";
              } else {
               $get_nip_dosen = $db->fetch_single_row("dosen","nidn",$nidn);
               $nip = $get_nip_dosen->nip;
                ...
                if ($check_dosen_kelas_exist==false) {
                   ...
                   $error_count++;
                   $error[] = "Dosen Ajar ".$kode_kelas." Mk ".$kode_mk." Sudah Ada";
                }
              }
            
            } else {
                  $error_count++;
                  $error[] = "Pastikan Kelas ".$kode_kelas." Mk ".$kode_mk." Sudah dibuat";
            }
...
> '".$_POST['jam_mulai']."' and jam_mulai <'".$_POST['jam_selesai']."' or jam_selesai>'".$_POST['jam_mulai']."' and jam_selesai<'".$_POST['jam_selesai']."' or jam_mulai='".$_POST['jam_mulai']."' and jam_selesai='".$_POST['jam_selesai']."') and hari like '%".$_POST['hari']."' and kelas_id!='".$_POST['kelas_id']."' and sem_id=? and ruang_id=? 
",$check_jadwal_kelas);
if ($check_bentrok_kelas->rowCount()>0) {
    foreach ($check_bentrok_kelas as $dt) {
      ...
      action_response("Maaf Ruangan ini Hari $dt->hari Jam ".$dt->jam_mulai." S/d ".$dt->jam_selesai." Sudah digunakan $prodi->jurusan Untuk  Matakuliah $dt->matkul_jadwal Kelas $dt->kelas_jadwal");
    }
}
...
$data = array_filter($data);
    ...
    foreach ($db->query("select * from komponen_nilai  where isShow='1' ") as $kp) { 
      if (isset($_POST['komponen_'.$kp->id])) {
         $data_komponen = array('id_kelas' => $_POST["kelas_id"],
                                'id_komponen' => $kp->id,
                                'nilai' => $_POST['komponen_'.$kp->id] );
        $in_kelas = $db->insert('kelas_penilaian',$data_komponen);
      }     
