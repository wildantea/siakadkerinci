...
<?php
  ...
  if(isset($_POST['priode_filter'])) {
    if ($_POST['priode_filter']!='all') {
      $priode = ' and jk.id_kompre="'.$_POST['priode_filter'].'"';
    }
  }
  ...
  $qq=$db->query("select k.nim,m.nama,f.nama_resmi,j.nama_jur,k.id from kompre k
    ...
    inner join jadwal_kompre jk on k.priode_kompre=jk.id_kompre 
    WHERE k.id IS NOT NULL $fakultas $jurusan $priode group by k.id");
foreach ($qq as $kk) {
  # code...
}
?>
<body onLoad="//window.print();">
  <div class="page-break">
    ...
    <br>
      <h2 align="center" style="margin-bottom: 10px;">DAFTAR PESERTA KOMPREHENSIF</h2>
...
<?php
  ...
  $q=$db->query("select k.nim,m.nama,f.nama_resmi,j.nama_jur,k.id from kompre k
    inner join mahasiswa m on k.nim=m.nim
    inner join fakultas f on k.kode_fak=f.kode_fak 
    inner join jurusan j on k.kode_jurusan=j.kode_jur
    inner join jadwal_kompre jk on k.priode_kompre=jk.id_kompre 
