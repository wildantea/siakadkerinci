...
$data_edit = $db->fetch_single_row("ruang_ref","ruang_id",$_POST['id_data']);
?>
  <style type="text/css"> .datepicker {z-index: 1200 !important; } </style>
   <div class="alert alert-danger error_data" style="display:none">
          ...
          <span class="isi_warning"></span>
            ...
            <form id="edit_ruang_kuliah" method="post" class="form-horizontal" action="<?=base_admin();?>modul/ruang_kuliah/ruang_kuliah_action.php?act=up">
              ...
              <select  id="gedung_id" name="gedung_id" data-placeholder="Pilih Gedung ..." class="form-control chzn-select" tabindex="2" required>
               ...
               <?php foreach ($db->fetch_all("gedung_ref") as $isi) {
                  ...
                  if ($data_edit->gedung_id==$isi->gedung_id) {
              ...
              <div class="form-group">
                ...
                <div class="col-lg-9">
                  <input type="text" name="kode_ruang" value="<?=$data_edit->kode_ruang;?>" class="form-control" required>
                </div>
              </div><!-- /.form-group -->
              <div class="form-group">
                <label for="Nama Ruang" class="control-label col-lg-3">Nama Ruang <span style="color:#FF0000">*</span></label>
                <div class="col-lg-9">
                  <input type="text" name="nm_ruang" value="<?=$data_edit->nm_ruang;?>" class="form-control" required>
                </div>
              </div><!-- /.form-group -->
              <div class="form-group">
                  ...
                  <select  id="kode_jur" name="kode_jur[]" data-placeholder="Bisa Multi Prodi  ..." class="form-control chzn-select" tabindex="2" required multiple="">
                     ...
                     <?php
                     $array_prodi = array();
                     $ruang_prodi = $db->query("select * from ruang_ref_prodi where ruang_id=?",array('ruang_id' => $data_edit->ruang_id));
                     if ($ruang_prodi->rowCount()>0) {
                       foreach ($ruang_prodi as $pr) {
                          $array_prodi[] = $pr->kode_jur;
                       }
                     }
                     foreach ($db->fetch_all("view_prodi_jenjang") as $isi) {
                      if (in_array($isi->kode_jur, $array_prodi)) {
              ...
              <input type="hidden" name="id" value="<?=$data_edit->ruang_id;?>">
