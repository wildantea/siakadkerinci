...
$sem = $_POST['sem'];
   ...
   $wangk = ""; 
                              if ($_POST['ang']!='all') {   
                                  $wangk = " AND m.`mulai_smt`='".de($_POST['ang'])."' ";  
                              } 

$qq = $db->query("SELECT fk.nama_resmi, j.kode_jur,j.nama_jur FROM akm a 
JOIN mahasiswa m ON m.`nim`=a.`mhs_nim`
JOIN jurusan j ON j.`kode_jur`=m.`jur_kode`
JOIN fakultas fk ON fk.kode_fak=j.fak_kode
WHERE j.`kode_jur`='$jur' ");
 foreach ($qq as $kk) {
  $jurusan = $kk->kode_jur;
 }
...
<body onLoad="//window.print();">
  ...
  <div class="page-break">
   ...
   <h3 align="center">DAFTAR REKAPITULASI AKTIVITAS KULIAH MAHASISWA</h3>
    <h4 align="center">Program Studi : <?= $kk->nama_jur ?></h4>
      ...
      <table class="tabel-common" width="100%">
         <tbody><tr>
            ...
            <th width="30" scope="col">SKS Diambil</th>
            <th width="30" scope="col">IP</th>
            <th width="30" scope="col">IPK</th>
            <th width="30" scope="col">Total SKS</th>
         </tr>

           
<?php
// print_r($_POST); 
// echo "select a.total_sks, m.nim,m.nama,m.stat_pd,a.sem_id,a.jatah_sks,a.ip, a.ipk, m.mhs_id FROM mahasiswa m
//                                 INNER JOIN akm a ON m.nim=a.mhs_nim
//                                 INNER JOIN jurusan j on m.jur_kode=j.kode_jur 
//                                 WHERE j.kode_jur='$jur' $wangk AND a.sem_id='$sem' ORDER BY a.mhs_nim asc";
      
          $q=$db->query("select a.total_sks, m.nim,m.nama,m.stat_pd,a.sem_id,a.jatah_sks,a.ip, a.ipk, m.mhs_id FROM mahasiswa m
                                INNER JOIN akm a ON m.nim=a.mhs_nim
                                INNER JOIN jurusan j on m.jur_kode=j.kode_jur 
                                WHERE j.kode_jur='$jur' $wangk AND a.sem_id='$sem' ORDER BY a.mhs_nim asc");
