                ...
                <section class="content-header">
                    <h1>
                       ...
                       <ol class="breadcrumb">
                        ...
                        <li><a href="<?=base_index();?>hak-akses-menu">Hak Akses Menu</a></li>
...
foreach ($db->query("select * from sys_group_level") as $isi) {

                  if ($_GET['user']==$isi->level) {
                     echo "<option value='$isi->level' selected>$isi->dekripsi</option>";
                  } else {
 ...
 <label for="Menu" class="control-label col-lg-2">&nbsp;</label>
                        <div class="col-lg-10">
<button style="margin-top:10px;margin-bottom:10px" class="btn btn-primary">Tampilkan Menu</button>
</div>
</form>
...
?>       
<h3>Checklist Untuk Memberikan Hak Akses Menu</h3>
<table id="dtb" class="table table-bordered table-condensed table-hover table-striped">
                      ...
                      <tbody>
                        <?php 
      $dtb=$db->query("select sys_group_users.level, sys_menu.type_menu,sys_menu_role.read_act,sys_menu_role.insert_act,sys_menu_role.update_act,sys_menu_role.delete_act, import_act,sys_menu.page_name,sys_menu.urutan_menu,sys_group_users.level_name,sys_menu_role.id from sys_menu_role inner join sys_menu on sys_menu_role.id_menu=sys_menu.id inner join sys_group_users on sys_menu_role.group_level=sys_group_users.level where sys_group_users.level=? group by sys_menu.id  order by urutan_menu asc",array('sys_group_users.level'=>$_GET
        ['user']));
      $i=1;
      foreach ($dtb as $isi) {
        ?><tr id="line_<?=$isi->id;?>">
