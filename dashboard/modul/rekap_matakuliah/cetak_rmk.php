...
<body onLoad="//window.print();">
  ...
  <div class="page-break">
   ...
   <h2 align="center">DAFTAR REKAPITULASI MATAKULAIH</h2>
    <h4 align="center">Program Studi :<?= $kk->nama_jur ?> S1 - Reguler</h4>
      ...
      <table class="tabel-common" width="100%">
         <tbody><tr>
            ...
            <th width="50" scope="col">NIM</th>
            <th width="150" scope="col">Nama</th>
            <th width="250" scope="col">Mata Kuliah</th>
            <th width="30" scope="col">Jumlah SKS</th>
...
<?php
          ...
          $q=$db->query("SELECT m.`nim`, m.`nama`,j.`nama_jur`, a.`jatah_sks`, a.ip, a.`sem_id`, m.`mulai_smt`,a.akm_id FROM akm a 
                                                    JOIN mahasiswa m ON m.`nim` = a.`mhs_nim`
                                                    JOIN jurusan j ON j.kode_jur=m.`jur_kode` 
                                                    WHERE j.`kode_jur`='$jur' AND m.`mulai_smt`='$ang' AND a.`sem_id`='$sem'");
          ...
          foreach ($q as $k) {
            $qd= $db->query("SELECT k.`mhs_id`, mt.`kode_mk`, mt.`nama_mk` FROM krs_detail kr
                                                      ...
                                                      where k.mhs_id='".$k->nim."' and s.`id_semester`='$sem' and kr.batal=0");
                                    foreach ($qd as $kd) {
