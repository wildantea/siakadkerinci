...
if(isset($_POST['lokasi_filter'])) {
  if ($_POST['lokasi_filter']!='all') {
    $lokasi = ' and lokasi_kkn.id_lokasi="'.$_POST['lokasi_filter'].'"';
  }
}
...
if(isset($_POST['priode_filter'])) {
  if ($_POST['priode_filter']!='all') {
    $priode = ' and priode_kkn.id_priode="'.$_POST['priode_filter'].'"';
  }
}
  ...
  <div class="nobreak">
    <table class="tabel-common" width="100%">
      <tbody>
        ...
        // echo "select kkn.nim,mahasiswa.nama,fakultas.nama_resmi,jurusan.nama_jur,kkn.id_kkn,lokasi_kkn.nama_lokasi from kkn 
        ...
        //   left join priode_kkn on priode_kkn.id_priode=kkn.id_priode
        //   left join lokasi_kkn on lokasi_kkn.id_lokasi=kkn.id_lokasi
        //   where id_kkn is not null $fakultas $jurusan $priode $lokasi $kelamin order by mahasiswa.nama asc, kkn.id_kkn";
        $q=$db->query("select kkn.nim,mahasiswa.nama,fakultas.nama_resmi,jurusan.nama_jur,kkn.id_kkn,lokasi_kkn.nama_lokasi from kkn 
          inner join mahasiswa on kkn.nim=mahasiswa.nim 
          inner join fakultas on kkn.kode_fak=fakultas.kode_fak 
          inner join jurusan on kkn.kode_jur=jurusan.kode_jur 
          left join priode_kkn on priode_kkn.id_priode=kkn.id_priode
