...
<section class="content">
  <div class="row">
        ...
        <div class="box-header">
          ...
          foreach ($db->fetch_all("sys_menu") as $isi) {
            if (uri_segment(1)==$isi->url) {
              if ($role_act["insert_act"]=="Y") {
                ...
                <a id="priode_pendaftaran_kukerta" class="btn btn-warning" href="<?=base_index();?>pendaftaran-kukerta/priode"><i class="fa fa-calendar-plus-o"></i> Setting Priode</a>

                <a id="lokasi_pendaftaran_kukerta" class="btn btn-warning" href="<?=base_index();?>pendaftaran-kukerta/lokasi"><i class="fa  fa-location-arrow"></i> Setting Lokasi</a>

                 <a id="setting_sks" class="btn btn-primary "><i class="fa fa-pencil"></i> Setting SKS</a>
                <?php
              }
            }
          }
          ...
          <div class="form-group">
            <span id="filter" class="btn btn-primary" style="margin-top: 20px;">
             ...
             <span id="filter" class="btn btn-success" style="margin-top: 20px;" onclick="cetak_nilai()">
    ...
    <a id="cetak_pendaftaran_kukerta" class="btn btn-success"><i class="fa fa-print"></i> Cetak Pendaftaran Kukerta</a>
...
<script type="text/javascript">
  ...
  function cetak_nilai() { 
     ...
     document.location="<?=base_admin();?>modul/pendaftaran_kukerta/cetak_nilai_kukerta.php?&fakultas="+fakultas+"&jurusan="+jurusan+"&priode="+priode+"&id_lokasi="+id_lokasi+"&jk="+jk;
  }
  ...
  function download_data() {
     ...
     var id_lokasi = $("#id_lokasi").val();
     var jk = $("input[name='jk']:checked").val(); 
     document.location="<?=base_admin();?>modul/pendaftaran_kukerta/pendaftaran_kukerta_action.php?act=download_data&fakultas="+fakultas+"&jurusan="+jurusan+"&priode="+priode+"&id_lokasi="+id_lokasi+"&jk="+jk;
 
  }

  function pilih_lokasi(id_periode) {
    // $.ajax({
    //   url : "<?=base_admin();?>modul/pendaftaran_kukerta/pendaftaran_kukerta_action.php?get_lokasi",
    //   type : "post",
    //   data : {
  ...
  $("#setting_sks").click(function() {
    $.ajax({
      url : "<?=base_admin();?>modul/pendaftaran_kukerta/setting_sks.php",
      type : "GET",
      success: function(data) {
   ...
   "ajax":{
     url :"<?=base_admin();?>modul/pendaftaran_kukerta/pendaftaran_kukerta_data.php",
      ...
      data: function ( d ) {
        ...
        d.jk = $("input[name='jk']:checked").val(); 
      },
      error: function (xhr, error, thrown) {
        console.log(xhr);
      }
