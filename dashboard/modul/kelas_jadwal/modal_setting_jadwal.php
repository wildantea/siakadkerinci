...
if ($check_exist_jadwal) {
  ...
  $id_jadwal_kuliah = $data_edit->jadwal_id;
  $jadwal = 1;
} else {
  $jadwal = 0;
}
...
<input type="hidden" name="kelas_name" value="<?=$kelas_data->nama_kelas;?>"> 
<input type="hidden" name="kode_jur" value="<?=$kelas_data->kode_jur;?>">  
<input type="hidden" name="semester" value="<?=$matkul->semester;?>">                    
...
//ruang_ref_prodi.kode_jur='$kelas_data->kode_jur' and
...
where  ruang_ref.is_aktif='Y' $kode_jur
                        ...
                        <select id="hari" name="hari" data-placeholder="Pilih Hari ..." class="form-control" tabindex="2" required="">
                          ...
                          if ($jadwal) {
                            foreach ($array_hari as $h => $hari) {
                              if (strtolower($data_edit->hari)==strtolower($h) or strtolower($data_edit->hari)==strtolower($hari)) {
...
$('.close_main').click(function() {
    ...
    function pilih_dosen(id_dosen,sks){
         $('#loadnya').show();
        $.ajax({
            type: 'POST',
            ...
            success: function(result) {
               $('#loadnya').hide();
              if (result=='Maaf SKS Dosen ini melebihi 16 SKS') {
                $('.isi_warning_sks_dosen').text(result);
                $('.error_data_sks_dosen').fadeIn();
              } else {
                $('.error_data_sks_dosen').hide();
