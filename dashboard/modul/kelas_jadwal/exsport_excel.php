...
$jur_filter="";
 // if (isset($_GET['jur_filter'])) {
  ...
  if ($_GET['fakultas']!='all') {
    $fakultas = getProdiFakultas('vnk.kode_jur',$_GET['fakultas']);
  }
...
if ($_SESSION['group_level']=='dosen') {
        $temp_rec = $db->query("select m.semester,m.total_sks as sks, r.kode_ruang,vnk.nama_kelas,m.nama_mk AS nama_matkul,vj.sem_id,vnk.kls_nama, vnk.kode_jur, vj.jam_mulai,vj.jam_selesai, vj.hari, vj.ruang_id, vnk.kls_nama, vnk.kode_mk,sem_matkul,nm_matkul,nama_kelas,vj.nm_ruang,vnk.id_matkul,vnk.nama_mk,vnk.nama_kelas,vj.waktu,vnk.peserta_max,vnk.jurusan,vnk.kelas_id,fungsi_get_jml_krs(vnk.kelas_id) as jml,fungsi_get_jml_krs_belum_disetujui(vnk.kelas_id) as belum_disetujui, fungsi_dosen_kelas(vnk.kelas_id) as nama_dosen,jenis_kelas.nama_jenis_kelas 
   from view_nama_kelas vnk
left join view_jadwal vj on vnk.kelas_id=vj.kelas_id
inner join jenis_kelas on vnk.id_jenis_kelas=jenis_kelas.id
LEFT JOIN matkul m ON m.id_matkul=vnk.id_matkul 
LEFT JOIN ruang_ref r ON r.ruang_id=vj.ruang_id
 left join dosen_kelas dk on dk.id_kelas=vnk.kelas_id     where vnk.kelas_id is not null $sem_filter $jur_filter  $hari_filter $matkul_filter $jenis_kelas $fakultas and dk.id_dosen='".$_SESSION['username']."' ");
  }else{
      $temp_rec = $db->query("select m.semester,m.total_sks as sks, r.kode_ruang,vnk.nama_kelas,m.nama_mk AS nama_matkul,vj.sem_id,vnk.kls_nama, vnk.kode_jur, vj.jam_mulai,vj.jam_selesai, vj.hari, vj.ruang_id, vnk.kls_nama, vnk.kode_mk,sem_matkul,nm_matkul,nama_kelas,vj.nm_ruang,vnk.id_matkul,vnk.nama_mk,vnk.nama_kelas,vj.waktu,vnk.peserta_max,vnk.jurusan,vnk.kelas_id,fungsi_get_jml_krs(vnk.kelas_id) as jml,fungsi_get_jml_krs_belum_disetujui(vnk.kelas_id) as belum_disetujui, fungsi_dosen_kelas(vnk.kelas_id) as nama_dosen,jenis_kelas.nama_jenis_kelas,
...
LEFT JOIN matkul m ON m.id_matkul=vnk.id_matkul 
LEFT JOIN ruang_ref r ON r.ruang_id=vj.ruang_id
     where vnk.kelas_id is not null $sem_filter $jur_filter  $hari_filter $matkul_filter $jenis_kelas $fakultas");
  }
