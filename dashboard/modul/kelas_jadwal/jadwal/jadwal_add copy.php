...
/*Right*/
  ...
  .modal.right.fade.in .modal-dialog {
    right: 0;
        ...
        .time-slot:hover {
            background-color: #5cb85c !important; /* Green background on hover */
            color: white !important; /* White text on hover */
        }
        .day-button:hover {
            background-color: #5cb85c !important; /* Green background on hover */
...
.schedule-container {
            max-width: 600px; /* Max width for the schedule */
            margin: auto auto; /* Center the container */
            background-color: #ffffff; /* White background for the schedule */
        ...
        .schedule-item {
            ...
            padding: 11px; /* Padding inside each item */
            margin-bottom: 10px; /* Space between items */
            position: relative; /* Position for the dot */
        }
        .schedule-item .time {
            font-weight: bold; /* Bold time text */
        }
        .schedule-item .dot {
        ...
        .work { background-color: #007bff; } /* Blue for work */
        .personal { background-color: #28a745; } /* Green for personal */
        .design { background-color: #dc3545; } /* Red for design */
</style>
...
<div class="row">
  <div class="col-lg-12" id="left_form">
      <form method="post" class="form-horizontal" id="input_jadwal" action="<?=base_admin();?>modul/kelas_jadwal/jadwal/jadwal_action.php?act=input_jadwal">
               ...
               <?php 
                $gedung = $db->query("select * from view_ruang_prodi where kode_jur=? and is_aktif='Y' group by gedung_id",array('kode_jur' => $_POST['kode_jur']));
                  foreach ($gedung as $isi) {
                    ...
                    <input type="radio" class="gedung_id" name="gedung_id" data-msg-required="Silakan Pilih Gedung" value="<?=$isi->gedung_id;?>" required> <?=$isi->nm_gedung;?>
                </label>
              </div>
              <?php
            }
        ...
        <div class="col-xs-12">
                ...
                <?php
                  ...
                  foreach ($array_hari as $h => $hari) {
                    ...
                    <input type="radio" class="hari_id" data-msg-required="Silakan Pilih Hari" required name="hari" value="<?=$h;?>"> <?=$hari;?>
                </label>
              </div>
                <?php
                            }  
...
<div class="col-lg-12 ">
    <div class="row">
        ...
        <span class="input-group-addon numbering drag-handle">1</span>
            <select name="pengajar[]" id="pengajar_1" data-placeholder="Pilih Dosen Pengajar..." class="form-control pengajar " tabindex="2" required data-msg-required="Silakan Pilih Pengajar">
              
              </select>
        </div>
    </div>
    <div class="col-lg-1 show-hapus-pengajar" style="padding-top:1px;padding-left:0;display: none;" data-toggle="tooltip" data-title="Hapus pengajar">
      <span class="btn btn-danger btn-sm hapus-pengajar"><i class="fa fa-trash"></i></span>
    </div>
    </div>
...
<div class="form-group">
<div class="col-lg-12">
<span class="btn btn-success btn-sm clone-btn"><i class="fa fa-plus"></i> Tambah Dosen Lain</span>
</div>
</div>
</div>
</div>
...
<div class="row">
            ...
            <div class="form-group">
              <div class="col-lg-12">
                <button type="button" class="btn btn-default cancel-modal-jadwal-add"><i class="fa fa-close"></i> Cancel</button>
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (result !== '') {
                    ...
                    $("#info-div").html('');
                    $("#left_form").removeClass().addClass('col-lg-12');
                    $("#right_form").removeClass().addClass('col-lg-0');
                }
            },
            error: function () {
                if (retryCount < maxRetries) {
