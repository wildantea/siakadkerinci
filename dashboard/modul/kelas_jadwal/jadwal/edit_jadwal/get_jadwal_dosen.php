...
$id_dos = implode(", ", $quotedArray);
 ...
 $dosen = $db->query("select * from dosen where nip in($id_dos) and nip in(select id_dosen from view_jadwal_dosen_kelas where hari=? and sem_id=? and id_kelas!='$kelas_id')",array('hari' => $hari,'sem_id' => $sem_id));
...
?>
        <div class="box box-primary box-solid" style="margin-bottom: 8px;">
            ...
            <div class="box-body" style="padding-bottom: 0;">
              <div class="row">
                <?php
$get_jadwal = $db->query("select *,(select jurusan from view_nama_kelas where kelas_id=view_jadwal_dosen_kelas.id_kelas) as jurusan  from view_jadwal_dosen_kelas where hari=? and sem_id=? and id_dosen='$dos->nip' and id_kelas!='$kelas_id' order by jam_mulai asc",array('hari' => $hari,'sem_id' => $sem_id));
