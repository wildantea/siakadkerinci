       ...
       if ($skala_nilai->rowCount()>0) {
            foreach ($skala_nilai as $skala) {
                  ...
                  if ( $bobot >=$min && $bobot <=$max) {
                        ...
                        $db->query("update konversi_matkul set nilai='$skala->nilai_huruf' where id='".$_POST['id']."' ");
                        $db->query("update konversi_matkul set bobot='$skala->nilai_indeks' where id='".$_POST['id']."'");
