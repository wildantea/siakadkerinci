...
switch ($_GET['act']) {
	...
	case 'back':
		...
		$data = $db->query("select group_level,read_act,insert_act,update_act,delete_act,import_act
	...
	if ($check_type->type_menu=='page') {
		if (file_exists("../../modul/".$db->fetch_single_row('sys_menu','id',$_GET['id'])->nav_act)) {
			$db->deleteDirectory("../../modul/".$db->fetch_single_row('sys_menu','id',$_GET['id'])->nav_act);
		}
		if (file_exists("../../../upload/".$db->fetch_single_row('sys_menu','id',$_GET['id'])->nav_act)) {
...
if(isset($_POST["tampil"])=="on")
    {
      ...
      $tampil = "N";
		...
		$up = $db->update('sys_menu',$data,'id',$_POST['id'],'no');
		if ($up=true) {
			echo "good";
		} else {
			return false;
