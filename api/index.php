...
function echoResponse($status_code, $response,$type) {
   ...
   /**
     ...
     */
  $authenticates = function ($type,$db,$access ) {
    return function () use ($type,$db,$access ) {
    ...
    if (isset($headers['Authorization'])) {
        ...
        if ($db->check_exist('sys_service_login',$data)) {
            return true; 
        } else {
  ...
  $authenticate = function ($type,$id_service,$access ) {
    ...
    if (isset($headers['Authorization'])) {
        ...
        $api_key = $headers['Authorization'];
         ...
         //get access 
        $array_access = array();
        $data_access = $db->fetch_single_row("sys_token","id_service",$id_service);
 ...
 /**
     ...
     * Checking if the request has valid api key in the 'Authorization' header
     */
  $otorisasi = function ($type,$id_service,$access ) {
    return function () use ($type,$id_service,$access ) {
